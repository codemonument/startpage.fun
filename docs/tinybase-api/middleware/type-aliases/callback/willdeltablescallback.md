...
The `WillDelTablesCallback` type describes a function that is called before all [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) are deleted from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
Multiple `WillDelTablesCallback` functions can be registered and they will be called sequentially. If any callback returns `false`, the chain short-circuits and the [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) will not be deleted.
## Since
v8.0.0
