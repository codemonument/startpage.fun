...
# `WillDelRowCallback`
The `WillDelRowCallback` type describes a function that is called before a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) is deleted from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `tableId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) containing the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/).  |  
| `rowId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) being deleted.  |  
| returns  | `boolean`  | `true` to allow the deletion, `false` to prevent it.  |  
The callback receives the table [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and row [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) about to be deleted. It returns `true` to allow the deletion, or `false` to prevent it.
Multiple `WillDelRowCallback` functions can be registered and they will be called sequentially. If any callback returns `false`, the chain short-circuits and the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) will not be deleted.
## Since
v8.0.0
