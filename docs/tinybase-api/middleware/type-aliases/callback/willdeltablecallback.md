...
The `WillDelTableCallback` type describes a function that is called before a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) is deleted from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
(tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)): boolean
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `tableId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) being deleted.  |  
| returns  | `boolean`  | `true` to allow the deletion, `false` to prevent it.  |  
The callback receives the table [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) about to be deleted. It returns `true` to allow the deletion, or `false` to prevent it.
Multiple `WillDelTableCallback` functions can be registered and they will be called sequentially. If any callback returns `false`, the chain short-circuits and the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) will not be deleted.
## Since
v8.0.0
