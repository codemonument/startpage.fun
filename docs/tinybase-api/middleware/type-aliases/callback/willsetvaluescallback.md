...
The `WillSetValuesCallback` type describes a function that is called before [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) are set in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
(values: Values[](https://tinybase.org/api/store/type-aliases/store/values/)): Values[](https://tinybase.org/api/store/type-aliases/store/values/) | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `values`  | `Values[](https://tinybase.org/api/store/type-aliases/store/values/)`  | The [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) object about to be set.  |  
| returns  | `Values[](https://tinybase.org/api/store/type-aliases/store/values/) | undefined`  | The [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) to use (possibly transformed), or `undefined` to prevent the write.  |  
The callback receives the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) object that is about to be set. It can return the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) (possibly transformed) to allow the write, or `undefined` to prevent the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) from being set.
Multiple `WillSetValuesCallback` functions can be registered and they will be called sequentially, the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) being updated successively. If any callback returns `undefined`, the chain short-circuits and the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) will not be set.
## Since
v8.0.0
