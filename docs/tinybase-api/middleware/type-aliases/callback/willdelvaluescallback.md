...
The `WillDelValuesCallback` type describes a function that is called before all [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) are deleted from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
Multiple `WillDelValuesCallback` functions can be registered and they will be called sequentially. If any callback returns `false`, the chain short-circuits and the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) will not be deleted.
## Since
v8.0.0
