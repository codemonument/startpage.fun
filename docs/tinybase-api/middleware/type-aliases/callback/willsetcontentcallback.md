...
The `WillSetContentCallback` type describes a function that is called before [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) is set in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
(content: Content[](https://tinybase.org/api/store/type-aliases/store/content/)): Content[](https://tinybase.org/api/store/type-aliases/store/content/) | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `content`  | `Content[](https://tinybase.org/api/store/type-aliases/store/content/)`  | The [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) about to be set.  |  
| returns  | `Content[](https://tinybase.org/api/store/type-aliases/store/content/) | undefined`  | The [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) to use (possibly transformed), or `undefined` to prevent the write.  |  
The callback receives the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) (a [Tables, Values] tuple) that is about to be set. It can return the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) (possibly transformed) to allow the write, or `undefined` to prevent the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) from being set.
Multiple `WillSetContentCallback` functions can be registered and they will be called sequentially, the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) being updated successively. If any callback returns `undefined`, the chain short-circuits and the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) will not be set.
## Since
v8.0.0
