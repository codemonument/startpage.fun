...
The `addWillApplyChangesCallback` method registers a [`WillApplyChangesCallback`](https://tinybase.org/api/middleware/type-aliases/callback/willapplychangescallback/) that will be called before [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) are applied to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) via the [`applyChanges`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/applychanges/) method.

```
addWillApplyChangesCallback[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/methods/configuration/addwillapplychangescallback/)(callback: WillApplyChangesCallback[](https://tinybase.org/api/middleware/type-aliases/callback/willapplychangescallback/)): Middleware[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `callback`  | `WillApplyChangesCallback[](https://tinybase.org/api/middleware/type-aliases/callback/willapplychangescallback/)`  | The [`WillApplyChangesCallback`](https://tinybase.org/api/middleware/type-aliases/callback/willapplychangescallback/) to register.  |  
| returns  | `Middleware[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/)`  | A reference to the [`Middleware`](https://tinybase.org/api/middleware/interfaces/middleware/middleware/) object, for chaining.  |  
This callback receives the [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) object and can return it (to allow), return a modified [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) object (to transform), or return `undefined` (to reject). Multiple callbacks can be registered and they are called sequentially, each receiving the output of the previous. If any callback returns `undefined`, all remaining callbacks are skipped and the changes are rejected.
This fires when applyChanges is called directly, or indirectly via applyMergeableChanges, setMergeableContent, or merge on a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/).
## Example
This example registers a callback that rejects changes containing a 'secret' table.
...
middleware.addWillApplyChangesCallback[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/methods/configuration/addwillapplychangescallback/)(([changedTables, changedValues]) =>
  changedTables['secret'] != null
    ? undefined
    : [changedTables, changedValues, 1],
);

store.applyChanges[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/applychanges/)([{pets: {fido: {species: 'dog'}}}, {}, 1]);
console.log(store.getRow[](https://tinybase.org/api/the-essentials/getting-data/getrow/)('pets', 'fido'));
// -> {species: 'dog'}

store.applyChanges[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/applychanges/)([{secret: {r1: {c1: 'v1'}}}, {}, 1]);
console.log(store.getTable[](https://tinybase.org/api/store/interfaces/store/store/methods/getter/gettable/)('secret'));
// -> {}

middleware.destroy[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/methods/lifecycle/destroy/)();
