...
The `addWillSetContentCallback` method registers a [`WillSetContentCallback`](https://tinybase.org/api/middleware/type-aliases/callback/willsetcontentcallback/) that will be called before [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) is set in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
addWillSetContentCallback[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/methods/configuration/addwillsetcontentcallback/)(callback: WillSetContentCallback[](https://tinybase.org/api/middleware/type-aliases/callback/willsetcontentcallback/)): Middleware[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `callback`  | `WillSetContentCallback[](https://tinybase.org/api/middleware/type-aliases/callback/willsetcontentcallback/)`  | The [`WillSetContentCallback`](https://tinybase.org/api/middleware/type-aliases/callback/willsetcontentcallback/) to register.  |  
...
The callback can transform the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) or return `undefined` to prevent the write. Multiple callbacks can be registered and they are called sequentially, each receiving the (possibly transformed) [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) from the previous callback.
## Example
This example registers a callback that prevents setting [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) with empty [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) in the pet store.
...
middleware.addWillSetContentCallback[](https://tinybase.org/api/middleware/interfaces/middleware/middleware/methods/configuration/addwillsetcontentcallback/)(([tables, values]) =>
  Object.keys(tables).length > 0 ? [tables, values] : undefined,
);

store.setContent[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcontent/)([{}, {open: true}]);
console.log(store.getContent[](https://tinybase.org/api/store/interfaces/store/store/methods/getter/getcontent/)());
// -> [{}, {}]

store.setContent[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcontent/)([{pets: {fido: {species: 'dog'}}}, {}]);
console.log(store.getContent[](https://tinybase.org/api/store/interfaces/store/store/methods/getter/getcontent/)());
