...
# `createIndexes`
The `createIndexes` function creates an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object, and is the main entry point into the [`indexes`](https://tinybase.org/api/indexes/) module.

```
createIndexes[](https://tinybase.org/api/indexes/functions/creation/createindexes/)(store: Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)): Indexes[](https://tinybase.org/api/indexes/interfaces/indexes/indexes/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `store`  | `Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)`  | The [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) for which to register [`Index`](https://tinybase.org/api/indexes/type-aliases/concept/index/) definitions.  |  
| returns  | `Indexes[](https://tinybase.org/api/indexes/interfaces/indexes/indexes/)`  | A reference to the new [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object.  |  
A given [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) can only have one [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object associated with it. If you call this function twice on the same [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), your second call will return a reference to the [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object created by the first.
## Examples
...
This example creates an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object, and calls the method a second time for the same [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to return the same object.

```
import {createIndexes, createStore} from 'tinybase';

const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)();
const indexes1 = createIndexes[](https://tinybase.org/api/indexes/functions/creation/createindexes/)(store);
const indexes2 = createIndexes[](https://tinybase.org/api/indexes/functions/creation/createindexes/)(store);
