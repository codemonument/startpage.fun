...
# `IndexIdsListener`
The `IndexIdsListener` type describes a function that is used to listen to [`Index`](https://tinybase.org/api/indexes/type-aliases/concept/index/) definitions being added or removed.

```
(indexes: Indexes[](https://tinybase.org/api/indexes/interfaces/indexes/indexes/)): void
```
...
A `IndexIdsListener` is provided when using the [`addIndexIdsListener`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/methods/listener/addindexidslistener/) method. See that method for specific examples.
When called, an `IndexIdsListener` is given a reference to the [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object.
## Since
v1.0.0
