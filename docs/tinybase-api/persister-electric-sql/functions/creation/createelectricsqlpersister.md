...
The `createElectricSqlPersister` function creates an [`ElectricSqlPersister`](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/) object that can persist a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a local ElectricSQL database.
...
| `electricClient`  | `ElectricClient<any>`  | The Electric client that was returned from `await electrify(...)`.  |  
...
| returns  | `ElectricSqlPersister[](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/)`  | A reference to the new [`ElectricSqlPersister`](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/) object.  |  
An [`ElectricSqlPersister`](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/) only supports regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects, and cannot be used to persist the metadata of a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/).
As well as providing a reference to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to persist, you must provide a `electricClient` parameter which identifies the Electric client.
...
## Examples
This example creates a [`ElectricSqlPersister`](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/) object and persists the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a local ElectricSql database as a JSON serialization into the `my_tinybase` table. It makes a change to the database directly and then reloads it back into the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
const persister = createElectricSqlPersister[](https://tinybase.org/api/persister-electric-sql/functions/creation/createelectricsqlpersister/)(
  store,
  electricClient,
  'my_tinybase',
);
...
This example creates a [`ElectricSqlPersister`](https://tinybase.org/api/persister-electric-sql/interfaces/persister/electricsqlpersister/) object and persists the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a local ElectricSql database with tabular mapping.
...
const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)().setTables[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settables/)({pets: {fido: {species: 'dog'}}});
const persister = createElectricSqlPersister[](https://tinybase.org/api/persister-electric-sql/functions/creation/createelectricsqlpersister/)(store, electricClient, {
  mode: 'tabular',
