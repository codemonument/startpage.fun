...
The `persister-react-native-sqlite` module of the TinyBase project lets you save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from a SQLite database using the [`react-native-sqlite-storage`](https://github.com/andpor/react-native-sqlite-storage) module (in an appropriate React Native environment).
## See also
