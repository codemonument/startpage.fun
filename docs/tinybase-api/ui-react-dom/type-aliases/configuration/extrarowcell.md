...
{
  label: string;
  component: ComponentType<RowProps[](https://tinybase.org/api/ui-react/type-aliases/props/rowprops/)>;
}
```
...
| `component`  | `ComponentType<RowProps[](https://tinybase.org/api/ui-react/type-aliases/props/rowprops/)>`  | A custom component for rendering each extra [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in the table, and which will be passed props for the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) with which it is placed.  |  
## Since
v6.6.0
