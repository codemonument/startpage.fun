...
{
  label: string;
  component: ComponentType<ValueProps[](https://tinybase.org/api/ui-react/type-aliases/props/valueprops/)>;
}
```
...
| `component`  | `ComponentType<ValueProps[](https://tinybase.org/api/ui-react/type-aliases/props/valueprops/)>`  | A custom component for rendering each extra [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in the table, and which will be passed props for the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) with which it is placed.  |  
## Since
v6.6.0
