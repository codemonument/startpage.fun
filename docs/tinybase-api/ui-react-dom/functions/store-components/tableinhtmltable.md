...
# `TableInHtmlTable`
The `TableInHtmlTable` component renders the contents of a single [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) in a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) as an HTML <table> element, and registers a listener so that any changes to that result will cause a re-render.

```
TableInHtmlTable[](https://tinybase.org/api/ui-react-dom/functions/store-components/tableinhtmltable/)(props: (
...
## Examples
This example creates a Provider context into which a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) is provided. The `TableInHtmlTable` component within it then renders the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) in a <table> element with a CSS class.
...
const Pane = () => <TableInHtmlTable[](https://tinybase.org/api/ui-react-dom/functions/store-components/tableinhtmltable/) tableId="pets" className="table" />;
