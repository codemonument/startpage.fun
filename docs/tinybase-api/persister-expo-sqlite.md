...
The `persister-expo-sqlite` module of the TinyBase project lets you save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from a Expo-SQLite database (in an appropriate React Native environment).
As of TinyBase v5.0, this module provides a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) for the modern version of Expo's [SQLite](https://docs.expo.dev/versions/latest/sdk/sqlite) library, designated 'next' as of November 2023 and default as v14.0 by June 2024.
Note that TinyBase support for the legacy version of Expo-SQLite is no longer available.
## See also
[Database Persistence](https://tinybase.org/guides/persistence/database-persistence/) guide
## Since
v4.5.0
## Interfaces
