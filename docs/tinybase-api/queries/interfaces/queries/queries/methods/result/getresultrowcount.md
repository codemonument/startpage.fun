...
# `getResultRowCount`
The `getResultRowCount` method returns the count of the [`ResultRow`](https://tinybase.org/api/queries/type-aliases/result/resultrow/) objects in the [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/) of the given query.

```
getResultRowCount[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/result/getresultrowcount/)(queryId: string): number
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `queryId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of a query.  |  
| returns  | `number`  | The number of [`ResultRow`](https://tinybase.org/api/queries/type-aliases/result/resultrow/) objects in the result of the query.  |  
This has the same behavior as a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/)'s [`getRowCount`](https://tinybase.org/api/store/interfaces/store/store/methods/getter/getrowcount/) method. For example, if the query [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) is invalid, the method returns zero.
## Example
This example creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, a single query definition, and then calls this method on it (as well as a non-existent definition) to get the [`ResultRow`](https://tinybase.org/api/queries/type-aliases/result/resultrow/) count.
...
console.log(queries.getResultRowCount[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/result/getresultrowcount/)('dogColors'));
// -> 2

console.log(queries.getResultRowCount[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/result/getresultrowcount/)('catColors'));
// -> 0
