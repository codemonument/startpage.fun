...
# `forEachQuery`
The `forEachQuery` method takes a function that it will then call for each Query in the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.

```
forEachQuery[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/iterator/foreachquery/)(queryCallback: QueryCallback[](https://tinybase.org/api/queries/type-aliases/callback/querycallback/)): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `queryCallback`  | `QueryCallback[](https://tinybase.org/api/queries/type-aliases/callback/querycallback/)`  | The function that should be called for every query.  |  
| returns  | `void`  | This has no return value.  |  
This method is useful for iterating over all the queries in a functional style. The `queryCallback` parameter is a [`QueryCallback`](https://tinybase.org/api/queries/type-aliases/callback/querycallback/) function that will be called with the [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of each query.
## Example
This example iterates over each query in a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.

```
import {createQueries, createStore} from 'tinybase';

const queries = createQueries[](https://tinybase.org/api/queries/functions/creation/createqueries/)(createStore[](https://tinybase.org/api/the-essentials/creating-stores/createstore/)())
...
queries.forEachQuery[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/iterator/foreachquery/)((queryId) => {
  console.log(queryId);
});
// -> 'dogColors'
// -> 'catColors'
