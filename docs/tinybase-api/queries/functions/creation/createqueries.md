...
# `createQueries`
The `createQueries` function creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, and is the main entry point into the [`queries`](https://tinybase.org/api/queries/) module.

```
createQueries[](https://tinybase.org/api/queries/functions/creation/createqueries/)(store: Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)): Queries[](https://tinybase.org/api/queries/interfaces/queries/queries/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `store`  | `Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)`  | The [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) for which to register query definitions.  |  
| returns  | `Queries[](https://tinybase.org/api/queries/interfaces/queries/queries/)`  | A reference to the new [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.  |  
...
## Examples
This example creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.
...
This example creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, and calls the method a second time for the same [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to return the same object.

```
import {createQueries, createStore} from 'tinybase';

const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)();
const queries1 = createQueries[](https://tinybase.org/api/queries/functions/creation/createqueries/)(store);
const queries2 = createQueries[](https://tinybase.org/api/queries/functions/creation/createqueries/)(store);
