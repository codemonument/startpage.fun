...
A `ResultCell` is typically accessed with the [`getResultCell`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/result/getresultcell/) method or [`addResultCellListener`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/listener/addresultcelllistener/) method. It is similar to the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) type in the [`store`](https://tinybase.org/api/store/) module, but without schema-specific typing, and is a JavaScript string, number, or boolean.
## Example
