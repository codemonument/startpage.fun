...
# `GroupedAs`
The `GroupedAs` type describes an object returned from calling a [`Group`](https://tinybase.org/api/queries/type-aliases/definition/group/) function so that the grouped [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) can be optionally aliased.

```
{as: (groupedCellId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)) => void}
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `as`  | `(groupedCellId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)) => void`  | A function that lets you specify an alias for the grouped [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).  |  
Note that if two [`Group`](https://tinybase.org/api/queries/type-aliases/definition/group/) clauses are both aliased to the same name (or if you create two groups of the same underlying [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/), both _without_ aliases), only the latter of two will be used in the query.
## Example
This example shows a query that groups the same underlying [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) twice, for different purposes. Both groups are aliased with the 'as' function to disambiguate them.
