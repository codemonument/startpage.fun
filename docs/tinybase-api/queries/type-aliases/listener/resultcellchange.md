...
# `ResultCellChange`
The `ResultCellChange` type describes a [`ResultCell`](https://tinybase.org/api/queries/type-aliases/result/resultcell/)'s changes during a transaction.

```
[changed: boolean, oldCell: ResultCellOrUndefined[](https://tinybase.org/api/queries/type-aliases/result/resultcellorundefined/), newCell: ResultCellOrUndefined[](https://tinybase.org/api/queries/type-aliases/result/resultcellorundefined/)]
```

This is returned by the [`GetResultCellChange`](https://tinybase.org/api/queries/type-aliases/listener/getresultcellchange/) function that is provided to every listener when called. This array contains the previous value of a [`ResultCell`](https://tinybase.org/api/queries/type-aliases/result/resultcell/) before the current transaction, the new value after it, and a convenience flag that indicates that the value has changed.
## Since
v2.0.0
