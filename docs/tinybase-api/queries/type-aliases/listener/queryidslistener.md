...
```
(queries: Queries[](https://tinybase.org/api/queries/interfaces/queries/queries/)): void
```
...
A `QueryIdsListener` is provided when using the [`addQueryIdsListener`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/listener/addqueryidslistener/) method. See that method for specific examples.
When called, a `QueryIdsListener` is given a reference to the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.
## Since
v2.0.0
