...
# `ResultTableCallback`
The `ResultTableCallback` type describes a function that takes a [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/)'s [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and a callback to loop over each [`ResultRow`](https://tinybase.org/api/queries/type-aliases/result/resultrow/) within it.

```
(
  tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/),
  forEachRow: (rowCallback: ResultRowCallback[](https://tinybase.org/api/queries/type-aliases/callback/resultrowcallback/)) => void,
): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `tableId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/) that the callback can operate on.  |  
| `forEachRow`  | `(rowCallback: ResultRowCallback[](https://tinybase.org/api/queries/type-aliases/callback/resultrowcallback/)) => void`  | A function that will let you iterate over the [`ResultRow`](https://tinybase.org/api/queries/type-aliases/result/resultrow/) objects in this [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/).  |  
| returns  | `void`  | This has no return value.  |  
A `ResultTableCallback` is provided when using the [`forEachResultTable`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/iterator/foreachresulttable/) method, so that you can do something based on every [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/) in the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object. See that method for specific examples.
## Since
v2.0.0
