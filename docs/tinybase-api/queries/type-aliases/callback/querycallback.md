...
# `QueryCallback`
The `QueryCallback` type describes a function that takes a query's [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).

```
(queryId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `queryId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the query that the callback can operate on.  |  
| returns  | `void`  | This has no return value.  |  
A `QueryCallback` is provided when using the [`forEachQuery`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/iterator/foreachquery/) method, so that you can do something based on every query in the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object. See that method for specific examples.
## Since
v2.0.0
