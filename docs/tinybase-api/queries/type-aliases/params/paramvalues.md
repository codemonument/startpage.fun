...
# `ParamValues`
The `ParamValues` type describes an object of param values, keyed by param [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), used to provide values for a parameterized query.

```
{[paramId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: ParamValue[](https://tinybase.org/api/queries/type-aliases/params/paramvalue/)}
```

A `ParamValues` object is provided when setting a query definition with params, or when updating params with the [`setParamValues`](https://tinybase.org/api/queries/interfaces/queries/queries/methods/configuration/setparamvalues/) method.
## Example
