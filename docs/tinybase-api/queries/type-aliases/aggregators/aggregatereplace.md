...
# `AggregateReplace`
The `AggregateReplace` type describes a function that can be used to optimize a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/) by providing a shortcut for when a single value in the input values is replaced with another.

```
(
  current: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/),
  add: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/),
  remove: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/),
  length: number,
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `current`  | `Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)`  | The current value of the aggregation.  |  
| `add`  | `Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)`  | The [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) value being added to the aggregation.  |  
| `remove`  | `Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)`  | The [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) value being removed from the aggregation.  |  
| `length`  | `number`  | The length of the array of [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values in the aggregation.  |  
| returns  | `ResultCellOrUndefined[](https://tinybase.org/api/queries/type-aliases/result/resultcellorundefined/)`  | The new value of the aggregation.  |  
...
When possible, if you are providing a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/), seek an implementation of an `AggregateReplace` function that can reduce the complexity cost of changing the input data set in place.
## Since
v2.0.0
