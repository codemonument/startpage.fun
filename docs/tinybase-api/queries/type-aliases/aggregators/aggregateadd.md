...
# `AggregateAdd`
The `AggregateAdd` type describes a function that can be used to optimize a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/) by providing a shortcut for when a single value is added to the input values.
...
When possible, if you are providing a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/), seek an implementation of an `AggregateAdd` function that can reduce the complexity cost of growing the input data set.
## Since
v2.0.0
