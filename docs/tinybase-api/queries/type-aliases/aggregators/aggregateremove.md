...
# `AggregateRemove`
The `AggregateRemove` type describes a function that can be used to optimize a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/) by providing a shortcut for when a single value is removed from the input values.
...
When possible, if you are providing a custom [`Aggregate`](https://tinybase.org/api/queries/type-aliases/aggregators/aggregate/), seek an implementation of an `AggregateRemove` function that can reduce the complexity cost of shrinking the input data set.
## Since
v2.0.0
