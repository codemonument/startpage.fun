      ...
      * [`mergeable-store`](https://tinybase.org/api/mergeable-store/)
        ...
        * [Type Aliases](https://tinybase.org/api/mergeable-store/type-aliases/)
          * [Mergeable type aliases](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/)
            * [`MergeableChanges`](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablechanges/)
            * [`MergeableContent`](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablecontent/)
...
# `MergeableChanges`
The `MergeableChanges` type represents changes to the content of a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) and the metadata about that content) required to merge it with another.

```
[mergeableTables: TablesStamp[](https://tinybase.org/api/mergeable-store/type-aliases/stamps/tablesstamp/)<Hashed>, mergeableValues: ValuesStamp[](https://tinybase.org/api/mergeable-store/type-aliases/stamps/valuesstamp/)<Hashed>, isChanges: 1]
```

It is simply an array of two [`Stamp`](https://tinybase.org/api/mergeable-store/type-aliases/stamps/stamp/) types, one for changes to the [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/)'s [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) and one for changes to its [`Values`](https://tinybase.org/api/store/type-aliases/store/values/). A final `1` is used to distinguish it from a full [`MergeableContent`](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablecontent/) object.
