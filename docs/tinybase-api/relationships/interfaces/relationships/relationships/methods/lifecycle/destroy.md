...
# `destroy`
The `destroy` method should be called when this [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object is no longer used.

```
destroy[](https://tinybase.org/api/relationships/interfaces/relationships/relationships/methods/lifecycle/destroy/)(): void
```

This guarantees that all of the listeners that the object registered with the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) are removed and it can be correctly garbage collected.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), adds a [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object with a definition (that registers a [`RowListener`](https://tinybase.org/api/store/type-aliases/listener/rowlistener/) with the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/)), and then destroys it again, removing the listener.
...
console.log(store.getListenerStats[](https://tinybase.org/api/relationships/interfaces/relationships/relationships/methods/development/getlistenerstats/)().row);
// -> 1

relationships.destroy[](https://tinybase.org/api/relationships/interfaces/relationships/relationships/methods/lifecycle/destroy/)();
