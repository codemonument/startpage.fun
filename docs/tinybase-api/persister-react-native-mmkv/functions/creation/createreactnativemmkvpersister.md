...
The `createReactNativeMmkvPersister` function creates a [`ReactNativeMmkvPersister`](https://tinybase.org/api/persister-react-native-mmkv/interfaces/persister/reactnativemmkvpersister/) object that can persist the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a local `react-native-mmkv` storage.
...
| returns  | `ReactNativeMmkvPersister[](https://tinybase.org/api/persister-react-native-mmkv/interfaces/persister/reactnativemmkvpersister/)`  | A reference to the new [`ReactNativeMmkvPersister`](https://tinybase.org/api/persister-react-native-mmkv/interfaces/persister/reactnativemmkvpersister/) object.  |  
A [`ReactNativeMmkvPersister`](https://tinybase.org/api/persister-react-native-mmkv/interfaces/persister/reactnativemmkvpersister/) supports both regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) and [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) objects.
As well as providing a reference to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to persist, you must provide a `storage` parameter which identifies the MMKV storage instance.
The third argument is a `storageName` string that configures which key to use for the storage key.
## Example
This example creates a [`ReactNativeMmkvPersister`](https://tinybase.org/api/persister-react-native-mmkv/interfaces/persister/reactnativemmkvpersister/) object and persists the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a MMKV storage instance as a JSON serialization into the `my_tinybase` key. It makes a change to the storage directly and then reloads it back into the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
const persister = createReactNativeMmkvPersister[](https://tinybase.org/api/persister-react-native-mmkv/functions/creation/createreactnativemmkvpersister/)(
  store,
  storage,
  'my_tinybase',
);
