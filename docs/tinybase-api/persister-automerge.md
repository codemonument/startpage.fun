...
# `persister-automerge`
The `persister-automerge` module of the TinyBase project provides a way to save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from an Automerge document.
A single entry point, the [`createAutomergePersister`](https://tinybase.org/api/persister-automerge/functions/creation/createautomergepersister/) function, is provided, which returns a new [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) object that can bind a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a provided Automerge document handle (and in turn, its document).
## See also
[Third-Party CRDT Persistence](https://tinybase.org/guides/persistence/third-party-crdt-persistence/) guide
## Since
v4.0.0
## Interfaces
