...
# `MetricAggregateAdd`
The `MetricAggregateAdd` type describes a function that can be used to optimize a custom [`MetricAggregate`](https://tinybase.org/api/metrics/type-aliases/aggregators/metricaggregate/) by providing a shortcut for when a single value is added to the input values.
...
| `add`  | `number`  | The number being added to the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/)'s aggregation.  |  
...
If it is not possible to shortcut the aggregation based on just one value being added, return `undefined` and the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) will be completely recalculated.
When possible, if you are providing a custom [`MetricAggregate`](https://tinybase.org/api/metrics/type-aliases/aggregators/metricaggregate/), seek an implementation of an `MetricAggregateAdd` function that can reduce the complexity cost of growing the input data set. See the [`setMetricDefinition`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/methods/configuration/setmetricdefinition/) method for more examples.
## Since
v1.0.0
