...
The `MetricAggregateRemove` type describes a function that can be used to optimize a custom [`MetricAggregate`](https://tinybase.org/api/metrics/type-aliases/aggregators/metricaggregate/) by providing a shortcut for when a single value is removed from the input values.

```
(
  metric: Metric[](https://tinybase.org/api/metrics/type-aliases/metric/metric/),
  remove: number,
  length: number,
): Metric[](https://tinybase.org/api/metrics/type-aliases/metric/metric/) | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `metric`  | `Metric[](https://tinybase.org/api/metrics/type-aliases/metric/metric/)`  | The current value of the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/).  |  
| `remove`  | `number`  | The number being removed from the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/)'s aggregation.  |  
| `length`  | `number`  | The length of the array of numbers in the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/)'s aggregation.  |  
| returns  | `Metric[](https://tinybase.org/api/metrics/type-aliases/metric/metric/) | undefined`  | The new value of the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/).  |  
...
If it is not possible to shortcut the aggregation based on just one value being removed, return `undefined` and the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) will be completely recalculated. One example might be if you were taking the minimum of the values, and the previous minimum is being removed. The whole of the rest of the list will need to be re-scanned to find a new minimum.
When possible, if you are providing a custom [`MetricAggregate`](https://tinybase.org/api/metrics/type-aliases/aggregators/metricaggregate/), seek an implementation of an `MetricAggregateRemove` function that can reduce the complexity cost of shrinking the input data set. See the [`setMetricDefinition`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/methods/configuration/setmetricdefinition/) method for more examples.
## Since
v1.0.0
