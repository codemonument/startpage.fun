...
# `persister-yjs`
The `persister-yjs` module of the TinyBase project provides a way to save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from a Yjs document.
A single entry point, the [`createYjsPersister`](https://tinybase.org/api/persister-yjs/functions/creation/createyjspersister/) function, is provided, which returns a new [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) object that can bind a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to a provided Yjs document.
## See also
