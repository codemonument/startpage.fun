...
# `ui-react-dom`
The `ui-react-dom` module of the TinyBase project provides components to make it easy to create web-based reactive apps with [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects.
The components in this module use the react-dom module and so are not appropriate for environments like React Native (although those in the lower-level [`ui-react`](https://tinybase.org/api/ui-react/) module are).
## See also
[UI Components (React)](https://tinybase.org/demos/ui-components-react/) demos
## Since
v4.1.0
## Functions
