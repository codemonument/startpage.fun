...
# `getClientIds`
The `getClientIds` method method returns the active clients that the [`WsServer`](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/) is handling for a given path.

```
getClientIds[](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/methods/getter/getclientids/)(pathId: string): Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `pathId`  | `string`  | The path for which to return the list of active clients.  |  
| returns  | `Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)`  | An array of the clients connected to the given path.  |  
## Example
This example creates a [`WsServer`](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/), sets some clients up to connect to it, and then gets the number of clients on the given paths. (The client [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) themselves are unique, based on the `sec-websocket-key` header.)
...
const synchronizer3 = await createWsSynchronizer[](https://tinybase.org/api/the-essentials/synchronizing-stores/createwssynchronizer/)(
  createMergeableStore[](https://tinybase.org/api/the-essentials/creating-stores/createmergeablestore/)(),
  new WebSocket('ws://localhost:8047/roomB'),
);

console.log(server.getClientIds[](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/methods/getter/getclientids/)('roomA').length);
// -> 2
console.log(server.getClientIds[](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/methods/getter/getclientids/)('roomB').length);
// -> 1
