        ...
        * [Interfaces](https://tinybase.org/api/checkpoints/interfaces/)
          * [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/)
            ...
            * [Lifecycle methods](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/lifecycle/)
              * [`clear`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/lifecycle/clear/)
              * [`clearForward`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/lifecycle/clearforward/)
              * [`destroy`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/lifecycle/destroy/)
...
# `clear`
The `clear` method resets this [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to its initial state, removing all the checkpoints it has been managing.

```
clear[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/lifecycle/clear/)(): Checkpoints[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/)
```
...
| --- | --- | --- |  
Obviously this method should be used with caution as it destroys the ability to undo or redo recent changes to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) (though of course the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) itself is not reset by this method).
This method can be useful when a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) is being loaded via a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) asynchronously after the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object has been attached, and you don't want users to be able to undo the initial load of the data. In this case you could call the `clear` method immediately after the initial load so that that is the baseline from which all subsequent changes are tracked.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, adds a listener, makes a change and then clears the checkpoints.
...
checkpoints.delListener[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/listener/dellistener/)(listenerId);
