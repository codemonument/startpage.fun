...
# `ExtraValueCell`
The `ExtraValueCell` object is used to configure the rendering of extra cells in an HTML table that's displaying [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) - before or after the regular data cells.

```
{
  label: string;
  component: Component<ValueViewProps[](https://tinybase.org/api/ui-svelte/type-aliases/props/valueviewprops/)>;
}
```
...
| `component`  | `Component<ValueViewProps[](https://tinybase.org/api/ui-svelte/type-aliases/props/valueviewprops/)>`  | A custom component for rendering each extra [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in the table, and which will be passed props for the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) with which it is placed.  |  
## Since
v6.6.0
