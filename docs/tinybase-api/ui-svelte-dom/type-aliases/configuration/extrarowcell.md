...
# `ExtraRowCell`
The `ExtraRowCell` object is used to configure the rendering of extra cells in an HTML table that's displaying a [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/) or [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) - before or after the regular data cells.

```
{
  label: string;
  component: Component<RowViewProps[](https://tinybase.org/api/ui-svelte/type-aliases/props/rowviewprops/)>;
}
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `label`  | `string`  | A string that will be used as the label at the top of the column for this [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/).  |  
| `component`  | `Component<RowViewProps[](https://tinybase.org/api/ui-svelte/type-aliases/props/rowviewprops/)>`  | A custom component for rendering each extra [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in the table, and which will be passed props for the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) with which it is placed.  |  
## Since
v6.6.0
