...
The `DatabasePersisterConfig` type describes the configuration of a database-oriented [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), such as those for SQLite and PostgreSQL.

```
DpcJson[](https://tinybase.org/api/persisters/type-aliases/configuration/dpcjson/) | DpcTabular[](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabular/)
```

There are two modes for persisting a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) with a database:
...
Please see the [`DpcJson`](https://tinybase.org/api/persisters/type-aliases/configuration/dpcjson/) and [`DpcTabular`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabular/) type documentation for more detail on each. If not specified otherwise, JSON serialization will be used for persistence.
[`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) made to the database (outside of this [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/)) are picked up immediately if they are made via the same connection or library that it is using. If the database is being changed by another client, the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) needs to poll for changes. Hence both configuration types also contain an `autoLoadIntervalSeconds` property which indicates how often it should do that. This defaults to 1 second.
Note that all the nested types within this type have a 'Dpc' prefix, short for '`DatabasePersisterConfig`'.
## Examples
When applied to a database [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), this `DatabasePersisterConfig` will load and save a JSON serialization from and to a table called `my_tinybase`, polling the database every 2 seconds. See [`DpcJson`](https://tinybase.org/api/persisters/type-aliases/configuration/dpcjson/) for more details on these settings.
...
When applied to a database [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), this `DatabasePersisterConfig` will load and save tabular data from and to tables specified in the `load` and `save` mappings. See [`DpcTabular`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabular/) for more details on these settings.
