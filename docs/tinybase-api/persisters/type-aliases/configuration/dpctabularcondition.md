...
# `DpcTabularCondition`
The `DpcTabularCondition` type describes the SQL WHERE clause that will be used to filter the rows that are loaded and saved to in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Table`](https://tinybase.org/api/store/type-aliases/store/table/).
