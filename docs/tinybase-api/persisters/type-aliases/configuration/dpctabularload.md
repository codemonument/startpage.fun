    ...
    * [API](https://tinybase.org/api/)
      ...
      * [`persisters`](https://tinybase.org/api/persisters/)
        ...
        * [Type Aliases](https://tinybase.org/api/persisters/type-aliases/)
          * [Listener type aliases](https://tinybase.org/api/persisters/type-aliases/listener/)
          * [Configuration type aliases](https://tinybase.org/api/persisters/type-aliases/configuration/)
            * [`DatabasePersisterConfig`](https://tinybase.org/api/persisters/type-aliases/configuration/databasepersisterconfig/)
            * [`DpcJson`](https://tinybase.org/api/persisters/type-aliases/configuration/dpcjson/)
            * [`DpcTabular`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabular/)
            * [`DpcTabularCondition`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabularcondition/)
            * [`DpcTabularLoad`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabularload/)
            * [`DpcTabularSave`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabularsave/)
            * [`DpcTabularValues`](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabularvalues/)
          ...
          * [Development type aliases](https://tinybase.org/api/persisters/type-aliases/development/)
...
# `DpcTabularLoad`
The `DpcTabularLoad` type describes the configuration for loading [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) in a database-oriented [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) that is operating in tabular mode.

```
{[tableName: string]: {
  tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/);
  rowIdColumnName?: string;
  condition?: DpcTabularCondition[](https://tinybase.org/api/persisters/type-aliases/configuration/dpctabularcondition/);
} | Id[](https://tinybase.org/api/common/type-aliases/identity/id/)}
```

It is an object where each key is a name of a database table, and the value is a child configuration object for how that table should be loaded into the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/). The properties of the child configuration object are:  
|   | Type  | Description  |  
| --- | --- | --- |  
| `tableId`  | [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/)  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) into which data from this database table should be loaded.  |  
| `rowIdColumnName?`  | string  | The optional name of the column in the database table that will be used as the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Table`](https://tinybase.org/api/store/type-aliases/store/table/), defaulting to '_id'.  |  
| `condition?`  | string  | The optional SQL WHERE clause that will be used to filter the rows that are loaded into the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Table`](https://tinybase.org/api/store/type-aliases/store/table/). When set it must include the `$tableName` placeholder for the table name, since v6.1.0.  |  
As a shortcut, if you do not need to specify a custom `rowIdColumnName`, you can simply provide the [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) instead of the whole object.
The 'Dpc' prefix indicates that this type is used within the [`DatabasePersisterConfig`](https://tinybase.org/api/persisters/type-aliases/configuration/databasepersisterconfig/) type.
## Example
When applied to a database [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), this [`DatabasePersisterConfig`](https://tinybase.org/api/persisters/type-aliases/configuration/databasepersisterconfig/) will load the data of two database tables (called 'petsInDb' and 'speciesInDb') into two [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) (called 'pets' and 'species'). One has a column for the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) called 'id' and the other defaults it to '_id'.
...
With the configuration above, this will load into a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) with [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) that look like this:
...
The example above represents what happens with a SQLite [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/). In PostgreSQL databases, all [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) and [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) columns are expected to be typed as `text`, and the strings, booleans, and numbers would be JSON-encoded if you queried them.
## Since
v4.0.0
