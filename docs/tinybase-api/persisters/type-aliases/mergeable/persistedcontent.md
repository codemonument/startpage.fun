...
# `PersistedContent`
The `PersistedContent` type is a generic representation of the content in the type of store being handled by a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/).

```
Persist extends Persists.StoreOrMergeableStore ? Content[](https://tinybase.org/api/store/type-aliases/store/content/) | MergeableContent[](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablecontent/) : Persist extends Persists.MergeableStoreOnly ? MergeableContent[](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablecontent/) : Content[](https://tinybase.org/api/store/type-aliases/store/content/)
```

Using the values of the [`Persists`](https://tinybase.org/api/persisters/enumerations/mergeable/persists/) enum, the generic parameter indicates whether the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) is handling content from a regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) (the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) type), a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) (the [`MergeableContent`](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablecontent/) type), or either (the union of the two).
If the generic parameter is unspecified, the StoreOnly enum value is used, meaning that `PersistedContent` is equivalent to the [`Content`](https://tinybase.org/api/store/type-aliases/store/content/) type.
## Since
v5.0.0
