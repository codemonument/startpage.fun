...
# `PersistedStore`
The `PersistedStore` type is a generic representation of the type of store being handled by a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/).

```
Persist extends Persists.StoreOrMergeableStore ? Store[](https://tinybase.org/api/the-essentials/creating-stores/store/) | MergeableStore[](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) : Persist extends Persists.MergeableStoreOnly ? MergeableStore[](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) : Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)
```

Using the values of the [`Persists`](https://tinybase.org/api/persisters/enumerations/mergeable/persists/) enum, the generic parameter indicates whether the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) is handling a regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/), or either.
If the generic parameter is unspecified, the StoreOnly enum value is used, meaning that `PersistedStore` is equivalent to a regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
## Since
v5.0.0
