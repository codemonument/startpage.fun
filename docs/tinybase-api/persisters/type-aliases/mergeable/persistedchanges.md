...
# `PersistedChanges`
The `PersistedChanges` type is a generic representation of changes made to the type of store being handled by a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/).

```
Persist extends Persists.StoreOrMergeableStore ? Changes[](https://tinybase.org/api/store/type-aliases/transaction/changes/) | MergeableChanges[](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablechanges/)<Hashed> : Persist extends Persists.MergeableStoreOnly ? MergeableChanges[](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablechanges/)<Hashed> : Changes[](https://tinybase.org/api/store/type-aliases/transaction/changes/)
```

Using the values of the [`Persists`](https://tinybase.org/api/persisters/enumerations/mergeable/persists/) enum, the generic parameter indicates whether the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) is handling changes for a regular [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) (the [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) type), a [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) (the [`MergeableChanges`](https://tinybase.org/api/mergeable-store/type-aliases/mergeable/mergeablechanges/) type), or either (the union of the two).
## Since
v5.0.0
