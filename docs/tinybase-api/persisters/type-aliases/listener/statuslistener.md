...
# `StatusListener`
The `StatusListener` type describes a function that is used to listen to changes to the loading and saving status of the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/).

```
(
  persister: Persister[](https://tinybase.org/api/the-essentials/persisting-stores/persister/),
  status: Status[](https://tinybase.org/api/persisters/enumerations/lifecycle/status/),
): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `persister`  | `Persister[](https://tinybase.org/api/the-essentials/persisting-stores/persister/)`  | A reference to the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) that changed.  |  
| `status`  | `Status[](https://tinybase.org/api/persisters/enumerations/lifecycle/status/)`  | The new loading or saving [`Status`](https://tinybase.org/api/persisters/enumerations/lifecycle/status/).  |  
| returns  | `void`  | This has no return value.  |  
A `StatusListener` is provided when using the [`addStatusListener`](https://tinybase.org/api/persisters/interfaces/persister/persister/methods/listener/addstatuslistener/) method. See that method for specific examples.
When called, a `StatusListener` is given a reference to the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) and the new [`Status`](https://tinybase.org/api/persisters/enumerations/lifecycle/status/): 0 means now idle, 1 means now loading, and 2 means now saving.
## Since
v5.3.0
