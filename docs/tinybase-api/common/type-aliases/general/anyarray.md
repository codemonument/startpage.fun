...
It is used to indicate that the value should be considered to be a JavaScript array, as can be stored in a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) or [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) with the `array` schema type.
## Since
v8.0.0
