...
# `Ids`
The `Ids` type is a simple alias for an array of strings, but is used to indicate that the strings should be considered to be the keys of objects (such as the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) strings used in a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/)).
