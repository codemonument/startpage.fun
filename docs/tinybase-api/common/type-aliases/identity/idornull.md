...
# `IdOrNull`
The `IdOrNull` type is a simple alias for the union of a string or `null` value, where the string should be considered to be the key of an objects (such as a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) string used in a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/)), and typically `null` indicates a wildcard - such as when used in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) [`addRowListener`](https://tinybase.org/api/the-essentials/listening-for-changes/addrowlistener/) method.

```
Id[](https://tinybase.org/api/common/type-aliases/identity/id/) | null
```
