...
# `Id`
The `Id` type is a simple alias for a string, but is used to indicate that the string should be considered to be the key of an object (such as a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) `Id` string used in a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/)).
