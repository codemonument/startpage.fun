...
# `CheckpointViewProps`
`CheckpointViewProps` props are used for components that refer to a single checkpoint in a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, such as the [`CheckpointView`](https://tinybase.org/api/ui-svelte/functions/component/checkpointview/) component.

```
{
  checkpointId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/);
  checkpoints?: CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-svelte/type-aliases/identity/checkpointsorcheckpointsid/);
  debugIds?: boolean;
}
```
