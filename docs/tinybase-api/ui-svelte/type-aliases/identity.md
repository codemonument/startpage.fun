[![TinyBase logo](https://tinybase.org/favicon.svg)TinyBase](https://tinybase.org/)
  1. No results found


  * [Guides](https://tinybase.org/guides/)
  * [Demos](https://tinybase.org/demos/)
  * [API](https://tinybase.org/api/)
  * [GitHub](https://github.com/tinyplex/tinybase)


  * [TinyBase](https://tinybase.org/)
    * [Guides](https://tinybase.org/guides/)
    * [Demos](https://tinybase.org/demos/)
    * [API](https://tinybase.org/api/)
      * [The Essentials](https://tinybase.org/api/the-essentials/)
      * [`store`](https://tinybase.org/api/store/)
      * [`mergeable-store`](https://tinybase.org/api/mergeable-store/)
      * [`metrics`](https://tinybase.org/api/metrics/)
      * [`indexes`](https://tinybase.org/api/indexes/)
      * [`relationships`](https://tinybase.org/api/relationships/)
      * [`queries`](https://tinybase.org/api/queries/)
      * [`checkpoints`](https://tinybase.org/api/checkpoints/)
      * [`common`](https://tinybase.org/api/common/)
      * [`persisters`](https://tinybase.org/api/persisters/)
      * [`persister-automerge`](https://tinybase.org/api/persister-automerge/)
      * [`persister-browser`](https://tinybase.org/api/persister-browser/)
      * [`persister-cr-sqlite-wasm`](https://tinybase.org/api/persister-cr-sqlite-wasm/)
      * [`persister-durable-object-sql-storage`](https://tinybase.org/api/persister-durable-object-sql-storage/)
      * [`persister-durable-object-storage`](https://tinybase.org/api/persister-durable-object-storage/)
      * [`persister-electric-sql`](https://tinybase.org/api/persister-electric-sql/)
      * [`persister-expo-sqlite`](https://tinybase.org/api/persister-expo-sqlite/)
      * [`persister-file`](https://tinybase.org/api/persister-file/)
      * [`persister-indexed-db`](https://tinybase.org/api/persister-indexed-db/)
      * [`persister-libsql`](https://tinybase.org/api/persister-libsql/)
      * [`persister-partykit-client`](https://tinybase.org/api/persister-partykit-client/)
      * [`persister-partykit-server`](https://tinybase.org/api/persister-partykit-server/)
      * [`persister-pglite`](https://tinybase.org/api/persister-pglite/)
      * [`persister-postgres`](https://tinybase.org/api/persister-postgres/)
      * [`persister-powersync`](https://tinybase.org/api/persister-powersync/)
      * [`persister-react-native-mmkv`](https://tinybase.org/api/persister-react-native-mmkv/)
      * [`persister-react-native-sqlite`](https://tinybase.org/api/persister-react-native-sqlite/)
      * [`persister-remote`](https://tinybase.org/api/persister-remote/)
      * [`persister-sqlite-bun`](https://tinybase.org/api/persister-sqlite-bun/)
      * [`persister-sqlite-wasm`](https://tinybase.org/api/persister-sqlite-wasm/)
      * [`persister-sqlite3`](https://tinybase.org/api/persister-sqlite3/)
      * [`persister-yjs`](https://tinybase.org/api/persister-yjs/)
      * [`synchronizers`](https://tinybase.org/api/synchronizers/)
      * [`synchronizer-broadcast-channel`](https://tinybase.org/api/synchronizer-broadcast-channel/)
      * [`synchronizer-local`](https://tinybase.org/api/synchronizer-local/)
      * [`synchronizer-ws-client`](https://tinybase.org/api/synchronizer-ws-client/)
      * [`synchronizer-ws-server`](https://tinybase.org/api/synchronizer-ws-server/)
      * [`synchronizer-ws-server-durable-object`](https://tinybase.org/api/synchronizer-ws-server-durable-object/)
      * [`synchronizer-ws-server-simple`](https://tinybase.org/api/synchronizer-ws-server-simple/)
      * [`ui-react`](https://tinybase.org/api/ui-react/)
      * [`ui-react-dom`](https://tinybase.org/api/ui-react-dom/)
      * [`ui-react-inspector`](https://tinybase.org/api/ui-react-inspector/)
      * [`ui-svelte`](https://tinybase.org/api/ui-svelte/)
        * [Functions](https://tinybase.org/api/ui-svelte/functions/)
        * [Type Aliases](https://tinybase.org/api/ui-svelte/type-aliases/)
          * [Identity type aliases](https://tinybase.org/api/ui-svelte/type-aliases/identity/)
            * [`StoreOrStoreId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/storeorstoreid/)
            * [`CheckpointsOrCheckpointsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/checkpointsorcheckpointsid/)
            * [`IndexesOrIndexesId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/indexesorindexesid/)
            * [`MaybeGetter`](https://tinybase.org/api/ui-svelte/type-aliases/identity/maybegetter/)
            * [`MetricsOrMetricsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/metricsormetricsid/)
            * [`PersisterOrPersisterId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/persisterorpersisterid/)
            * [`QueriesOrQueriesId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/queriesorqueriesid/)
            * [`RelationshipsOrRelationshipsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/relationshipsorrelationshipsid/)
            * [`SynchronizerOrSynchronizerId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/synchronizerorsynchronizerid/)
          * [Props type aliases](https://tinybase.org/api/ui-svelte/type-aliases/props/)
      * [`ui-svelte-dom`](https://tinybase.org/api/ui-svelte-dom/)
      * [`ui-svelte-inspector`](https://tinybase.org/api/ui-svelte-inspector/)
      * [`middleware`](https://tinybase.org/api/middleware/)
      * [`schematizer-arktype`](https://tinybase.org/api/schematizer-arktype/)
      * [`schematizer-effect`](https://tinybase.org/api/schematizer-effect/)
      * [`schematizer-typebox`](https://tinybase.org/api/schematizer-typebox/)
      * [`schematizer-valibot`](https://tinybase.org/api/schematizer-valibot/)
      * [`schematizer-yup`](https://tinybase.org/api/schematizer-yup/)
      * [`schematizer-zod`](https://tinybase.org/api/schematizer-zod/)
      * [`schematizers`](https://tinybase.org/api/schematizers/)


  * [TinyBase](https://tinybase.org/)
  * [API](https://tinybase.org/api/)
  * [`ui-svelte`](https://tinybase.org/api/ui-svelte/)
  * [Type Aliases](https://tinybase.org/api/ui-svelte/type-aliases/)
  * [Identity type aliases](https://tinybase.org/api/ui-svelte/type-aliases/identity/)


# Identity type aliases
## `StoreOrStoreId`
The [`StoreOrStoreId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/storeorstoreid/) type is used when you need to refer to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/storeorstoreid/).
## `CheckpointsOrCheckpointsId`
The [`CheckpointsOrCheckpointsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/checkpointsorcheckpointsid/) type is used when you need to refer to a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/checkpointsorcheckpointsid/).
## `IndexesOrIndexesId`
The [`IndexesOrIndexesId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/indexesorindexesid/) type is used when you need to refer to an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/indexesorindexesid/).
## `MaybeGetter`
The [`MaybeGetter`](https://tinybase.org/api/ui-svelte/type-aliases/identity/maybegetter/) type represents a value that can be provided either as a plain value or as a reactive getter function. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/maybegetter/).
## `MetricsOrMetricsId`
The [`MetricsOrMetricsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/metricsormetricsid/) type is used when you need to refer to a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/metricsormetricsid/).
## `PersisterOrPersisterId`
The [`PersisterOrPersisterId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/persisterorpersisterid/) type is used when you need to refer to a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/persisterorpersisterid/).
## `QueriesOrQueriesId`
The [`QueriesOrQueriesId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/queriesorqueriesid/) type is used when you need to refer to a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/queriesorqueriesid/).
## `RelationshipsOrRelationshipsId`
The [`RelationshipsOrRelationshipsId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/relationshipsorrelationshipsid/) type is used when you need to refer to a [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/relationshipsorrelationshipsid/).
## `SynchronizerOrSynchronizerId`
The [`SynchronizerOrSynchronizerId`](https://tinybase.org/api/ui-svelte/type-aliases/identity/synchronizerorsynchronizerid/) type is used when you need to refer to a [`Synchronizer`](https://tinybase.org/api/the-essentials/synchronizing-stores/synchronizer/) object in a ui-svelte function or component. [Read more](https://tinybase.org/api/ui-svelte/type-aliases/identity/synchronizerorsynchronizerid/).
[GitHub](https://github.com/tinyplex/tinybase)[Bluesky](https://bsky.app/profile/tinybase.bsky.social)[X / Twitter](https://x.com/tinybasejs)[Discord](https://discord.com/invite/mGz3mevwP8)[TinyBase v8.2.0](https://tinybase.org/) © 2022-
