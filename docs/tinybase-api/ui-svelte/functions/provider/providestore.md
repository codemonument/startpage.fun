...
# `provideStore`
The `provideStore` function registers a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) with a given [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) into the current Provider context, making it available to all descendant components.
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `storeId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) to register the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) under.  |  
| `store`  | `Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)`  | The [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to register.  |  
| returns  | `void`  | This has no return value.  |  
The `provideStore` function must be called inside a Svelte component's `<script>` block that is a descendant of a [`Provider`](https://tinybase.org/api/ui-svelte/functions/component/provider/) component.
## Since
v8.1.0
