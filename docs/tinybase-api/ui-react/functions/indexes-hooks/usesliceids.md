...
# `useSliceIds`
The `useSliceIds` hook gets the list of [`Slice`](https://tinybase.org/api/indexes/type-aliases/concept/slice/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in an [`Index`](https://tinybase.org/api/indexes/type-aliases/concept/index/), and registers a listener so that any changes to that result will cause a re-render.

```
useSliceIds[](https://tinybase.org/api/ui-react/functions/indexes-hooks/usesliceids/)(
  indexId: string,
  indexesOrIndexesId?: IndexesOrIndexesId[](https://tinybase.org/api/ui-react/type-aliases/identity/indexesorindexesid/),
): Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `indexId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Index`](https://tinybase.org/api/indexes/type-aliases/concept/index/).  |  
...
| returns  | `Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)`  | The [`Slice`](https://tinybase.org/api/indexes/type-aliases/concept/slice/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Index`](https://tinybase.org/api/indexes/type-aliases/concept/index/), or an empty array.  |  
A [`Provider`](https://tinybase.org/api/ui-react/functions/context-components/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object or a set of [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useSliceIds` hook lets you indicate which [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object to get data for: omit the optional final parameter for the default context [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object, or provide a [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object explicitly by reference.
When first rendered, this hook will create a listener so that changes to the [`Slice`](https://tinybase.org/api/indexes/type-aliases/concept/slice/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) will cause a re-render. When the component containing this hook is unmounted, the listener will be automatically removed.
## Examples
This example creates an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object outside the application, which is used in the `useSliceIds` hook by reference. A change to the [`Slice`](https://tinybase.org/api/indexes/type-aliases/concept/slice/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) re-renders the component.
...
This example creates a Provider context into which a default [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object is provided. A component within it then uses the `useSliceIds` hook.
...
const Pane = () => <span>{JSON.stringify(useSliceIds[](https://tinybase.org/api/ui-react/functions/indexes-hooks/usesliceids/)('bySpecies'))}</span>;

const indexes = createIndexes[](https://tinybase.org/api/indexes/functions/creation/createindexes/)(
...
).setIndexDefinition[](https://tinybase.org/api/indexes/interfaces/indexes/indexes/methods/configuration/setindexdefinition/)('bySpecies', 'pets', 'species');
...
const App = ({indexes}) => (
  <Provider[](https://tinybase.org/api/ui-react/functions/context-components/provider/) indexesById={{petIndexes: indexes}}>
    <Pane />
  </Provider[](https://tinybase.org/api/ui-react/functions/context-components/provider/)>
);
const Pane = () => (
  <span>{JSON.stringify(useSliceIds[](https://tinybase.org/api/ui-react/functions/indexes-hooks/usesliceids/)('bySpecies', 'petIndexes'))}</span>
);
