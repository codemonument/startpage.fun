...
The `usePersisterOrPersisterById` hook is used to get a reference to a [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object from within a [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component context, _or_ have it passed directly to this hook.

```
usePersisterOrPersisterById[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterorpersisterbyid/)(persisterOrPersisterId?: PersisterOrPersisterId[](https://tinybase.org/api/ui-react/type-aliases/identity/persisterorpersisterid/)): AnyPersister[](https://tinybase.org/api/persisters/type-aliases/mergeable/anypersister/) | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `persisterOrPersisterId?`  | `PersisterOrPersisterId[](https://tinybase.org/api/ui-react/type-aliases/identity/persisterorpersisterid/)`  | Either an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for accessing a [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object that was named with an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in the Provider, or the [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object itself.  |  
...
This is mostly of use when you are developing a component that needs a [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object and which might have been passed in explicitly to the component or is to be picked up from the context by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) (a common pattern for [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/)-based components).
This is unlikely to be used often. For most situations, you will want to use the [`usePersister`](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersister/) hook.
## Example
This example creates a Provider context into which a default [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object is provided. A component within it then uses the `usePersisterOrPersisterById` hook to get a reference to the [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object again, without the need to have it passed as a prop. Note however, that unlike the [`usePersister`](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersister/) hook example, this component would also work if you were to pass the [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) object directly into it, making it more portable.
...
const Pane = ({persister}) => (
  <span>{usePersisterOrPersisterById[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterorpersisterbyid/)(persister).getStatus[](https://tinybase.org/api/persister-browser/interfaces/persister/sessionpersister/methods/lifecycle/getstatus/)()}</span>
);
