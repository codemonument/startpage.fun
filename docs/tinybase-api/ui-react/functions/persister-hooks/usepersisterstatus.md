...
# `usePersisterStatus`
The `usePersisterStatus` hook returns a the status of a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), and registers a listener so that any changes to it will cause a re-render.

```
usePersisterStatus[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterstatus/)(persisterOrPersisterId?: PersisterOrPersisterId[](https://tinybase.org/api/ui-react/type-aliases/identity/persisterorpersisterid/)): Status[](https://tinybase.org/api/persisters/enumerations/lifecycle/status/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `persisterOrPersisterId?`  | `PersisterOrPersisterId[](https://tinybase.org/api/ui-react/type-aliases/identity/persisterorpersisterid/)`  | The [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) to be accessed: omit for the default context [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), or provide an explicit reference.  |  
| returns  | `Status[](https://tinybase.org/api/persisters/enumerations/lifecycle/status/)`  | The status of the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/): 0 means idle, 1 means loading, and 2 means saving.  |  
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) or a set of [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `usePersisterStatus` hook lets you indicate which [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) to get data for: omit the optional parameter for the default context [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/), or provide a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) explicitly by reference.
When first rendered, this hook will create a listener so that changes to the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) status will cause a re-render. When the component containing this hook is unmounted, the listener will be automatically removed.
## Examples
This example creates a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) outside the application, which is used in the `usePersisterStatus` hook by reference. A change to the status of the [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) re-renders the component.
...
const persister = createSessionPersister[](https://tinybase.org/api/persister-browser/functions/creation/createsessionpersister/)(createStore[](https://tinybase.org/api/the-essentials/creating-stores/createstore/)(), 'pets');
const App = () => <span>{usePersisterStatus[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterstatus/)(persister)}</span>;
...
This example creates a Provider context into which a default [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) is provided. A component within it then uses the `usePersisterStatus` hook.
...
const App = ({persister}) => (
  <Provider[](https://tinybase.org/api/the-essentials/using-react/provider/) persister={persister}>
    <Pane />
  </Provider[](https://tinybase.org/api/the-essentials/using-react/provider/)>
);
const Pane = () => <span>{usePersisterStatus[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterstatus/)()}</span>;
...
This example creates a Provider context into which a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) is provided, named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). A component within it then uses the `usePersisterStatus` hook.
...
const App = ({persister}) => (
  <Provider[](https://tinybase.org/api/the-essentials/using-react/provider/) persistersById={{petPersister: persister}}>
    <Pane />
  </Provider[](https://tinybase.org/api/the-essentials/using-react/provider/)>
);
const Pane = () => <span>{usePersisterStatus[](https://tinybase.org/api/ui-react/functions/persister-hooks/usepersisterstatus/)('petPersister')}</span>;
