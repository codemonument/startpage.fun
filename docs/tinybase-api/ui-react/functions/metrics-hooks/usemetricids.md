...
# `useMetricIds`
The `useMetricIds` hook gets an array of the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) registered with a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, and registers a listener so that any changes to that result will cause a re-render.

```
useMetricIds[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetricids/)(metricsOrMetricsId?: MetricsOrMetricsId[](https://tinybase.org/api/ui-react/type-aliases/identity/metricsormetricsid/)): Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)
```
...
| returns  | `Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)`  | The [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, or an empty array.  |  
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object or a set of [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useMetricIds` hook lets you indicate which [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to get data for: omit the optional final parameter for the default context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, or provide a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object explicitly by reference.
When first rendered, this hook will create a listener so that changes to the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object will cause a re-render. When the component containing this hook is unmounted, the listener will be automatically removed.
## Example
This example creates an [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object outside the application, which is used in the `useMetricIds` hook by reference. A newly-registered [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) re-renders the component.
...
const App = () => <span>{JSON.stringify(useMetricIds[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetricids/)(metrics))}</span>;
