...
# `useMetricsIds`
The `useMetricsIds` hook is used to retrieve the [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) of all the named [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) objects present in the current [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component context.

```
useMetricsIds[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetricsids/)(): Ids[](https://tinybase.org/api/common/type-aliases/identity/ids/)
```
...
| --- | --- | --- |  
## Example
This example adds two named [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) objects to a Provider context and an inner component accesses their [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/).
...
const App = () => {
  ...
  const metrics1 = useCreateMetrics[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usecreatemetrics/)(store1, createMetrics);
  const store2 = useCreateStore[](https://tinybase.org/api/ui-react/functions/store-hooks/usecreatestore/)(createStore);
  const metrics2 = useCreateMetrics[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usecreatemetrics/)(store2, createMetrics);
  return (
    <Provider[](https://tinybase.org/api/the-essentials/using-react/provider/) metricsById={{metrics1, metrics2}}>
      <Pane />
    </Provider[](https://tinybase.org/api/the-essentials/using-react/provider/)>
  );
};
const Pane = () => <span>{JSON.stringify(useMetricsIds[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetricsids/)())}</span>;
