...
|   | Type  | Description  |  
| --- | --- | --- |  
| `metricsId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to be registered with the Provider.  |  
| `metrics`  | `Metrics[](https://tinybase.org/api/metrics/interfaces/metrics/metrics/)`  | The [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to be registered.  |  
| returns  | `void`  | This has no return value.  |  
Normally you will register a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in a context by using the `metricsById` prop of the top-level [`Provider`](https://tinybase.org/api/ui-react/functions/context-components/provider/) component. This hook, however, lets you dynamically add a new [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to the context, from within a descendent component. This is useful for applications where the set of [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) objects is not known at the time of the first render of the root Provider.
A [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object added to the Provider context in this way will be available to other components within the context (using the [`useMetrics`](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetrics/) hook and so on). If you use the same [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) as an existing [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object registration, the new one will take priority over one provided by the `metricsById` prop.
Note that other components that consume a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object registered like this should defend against it being undefined at first. On the first render, the other component will likely not yet have completed the registration. In the example below, we use the null-safe `useMetrics('petMetrics')?` to do this.
## Example
This example creates a Provider context. A child component registers a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object into it which is then consumable by a peer child component.
...
const RegisterMetrics = () => {
  ...
  const metrics = useCreateMetrics[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usecreatemetrics/)(store, (store) =>
    createMetrics[](https://tinybase.org/api/metrics/functions/creation/createmetrics/)(store).setMetricDefinition[](https://tinybase.org/api/metrics/interfaces/metrics/metrics/methods/configuration/setmetricdefinition/)('petCount', 'pets', 'count'),
  );
  useProvideMetrics[](https://tinybase.org/api/ui-react/functions/metrics-hooks/useprovidemetrics/)('petMetrics', metrics);
  return null;
};
const ConsumeMetrics = () => (
  <span>{useMetrics[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetrics/)('petMetrics')?.getMetric[](https://tinybase.org/api/metrics/interfaces/metrics/metrics/methods/getter/getmetric/)('petCount')}</span>
);
