...
# `useMetric`
The `useMetric` hook gets the current value of a [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/), and registers a listener so that any changes to that result will cause a re-render.

```
useMetric[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetric/)(
  metricId: string,
  metricsOrMetricsId?: MetricsOrMetricsId[](https://tinybase.org/api/ui-react/type-aliases/identity/metricsormetricsid/),
): number | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `metricId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/).  |  
| `metricsOrMetricsId?`  | `MetricsOrMetricsId[](https://tinybase.org/api/ui-react/type-aliases/identity/metricsormetricsid/)`  | The [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to be accessed: omit for the default context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, or provide an explicit reference.  |  
| returns  | `number | undefined`  | The numeric value of the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/), or `undefined`.  |  
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object or a set of [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useMetric` hook lets you indicate which [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object to get data for: omit the optional final parameter for the default context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object, or provide a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object explicitly by reference.
When first rendered, this hook will create a listener so that changes to the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) will cause a re-render. When the component containing this hook is unmounted, the listener will be automatically removed.
## Examples
This example creates a [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object outside the application, which is used in the `useMetric` hook by reference. A change to the [`Metric`](https://tinybase.org/api/metrics/type-aliases/metric/metric/) re-renders the component.
...
This example creates a Provider context into which a default [`Metrics`](https://tinybase.org/api/metrics/interfaces/metrics/metrics/) object is provided. A component within it then uses the `useMetric` hook.
...
const Pane = () => <span>{useMetric[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetric/)('highestPrice')}</span>;

const metrics = createMetrics[](https://tinybase.org/api/metrics/functions/creation/createmetrics/)(
...
).setMetricDefinition[](https://tinybase.org/api/metrics/interfaces/metrics/metrics/methods/configuration/setmetricdefinition/)('highestPrice', 'species', 'max', 'price');
...
const App = ({metrics}) => (
  <Provider[](https://tinybase.org/api/the-essentials/using-react/provider/) metricsById={{petMetrics: metrics}}>
    <Pane />
  </Provider[](https://tinybase.org/api/the-essentials/using-react/provider/)>
);
const Pane = () => <span>{useMetric[](https://tinybase.org/api/ui-react/functions/metrics-hooks/usemetric/)('highestPrice', 'petMetrics')}</span>;
