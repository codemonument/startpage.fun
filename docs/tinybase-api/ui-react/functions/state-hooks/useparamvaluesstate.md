...
# `useParamValuesState`
The `useParamValuesState` hook returns an array containing all the parameter values for a query, and a callback for changing them, providing an easy way to bind a query's parameters to a user-controlled component.

```
useParamValuesState[](https://tinybase.org/api/ui-react/functions/state-hooks/useparamvaluesstate/)(
  queryId: string,
  queriesOrQueriesId?: QueriesOrQueriesId[](https://tinybase.org/api/ui-react/type-aliases/identity/queriesorqueriesid/),
): [ParamValues[](https://tinybase.org/api/queries/type-aliases/params/paramvalues/), (paramValues: ParamValues[](https://tinybase.org/api/queries/type-aliases/params/paramvalues/)) => void]
```
...
| returns  | `[ParamValues[](https://tinybase.org/api/queries/type-aliases/params/paramvalues/), (paramValues: ParamValues[](https://tinybase.org/api/queries/type-aliases/params/paramvalues/)) => void]`  | An array containing the parameter values and a function to set them.  |  
This is a convenience hook that combines the useParamValues and [`useSetParamValuesCallback`](https://tinybase.org/api/ui-react/functions/queries-hooks/usesetparamvaluescallback/) hooks. It's useful when you need both read and write access to query parameters in a single component.
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object or a set of [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useParamValuesState` hook lets you indicate which [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object to use: omit the final parameter for the default context [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, or provide an explicit reference.
## Example
This example creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object outside the application, which is used in the `useParamValuesState` hook by reference. A button updates the parameters when clicked.
...
const App = () => {
  const [paramValues, setParamValues] = useParamValuesState[](https://tinybase.org/api/ui-react/functions/state-hooks/useparamvaluesstate/)(
    'petsBySpecies',
    queries,
  );
  return (
    <button onClick={() => setParamValues[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/configuration/setparamvalues/)({species: 'cat'})}>
