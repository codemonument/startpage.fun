...
# `useValuesState`
The `useValuesState` hook returns a [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) object and a function to set it, following the same pattern as React's useState hook.

```
useValuesState[](https://tinybase.org/api/ui-react/functions/state-hooks/usevaluesstate/)(storeOrStoreId?: StoreOrStoreId[](https://tinybase.org/api/ui-react/type-aliases/identity/storeorstoreid/)): [Values[](https://tinybase.org/api/store/type-aliases/store/values/), (values: Values[](https://tinybase.org/api/store/type-aliases/store/values/)) => void]
```
...
| returns  | `[Values[](https://tinybase.org/api/store/type-aliases/store/values/), (values: Values[](https://tinybase.org/api/store/type-aliases/store/values/)) => void]`  | An array containing the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) object and a function to set it.  |  
This is a convenience hook that combines the useValues and [`useSetValuesCallback`](https://tinybase.org/api/ui-react/functions/store-hooks/usesetvaluescallback/) hooks. It's useful when you need both read and write access to all [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) in a single component.
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) or a set of [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useValuesState` hook lets you indicate which [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to use: omit the parameter for the default context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), or provide a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) explicitly by reference.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) outside the application, which is used in the `useValuesState` hook by reference. A button updates the [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) when clicked.
...
const App = () => {
  const [values, setValues] = useValuesState[](https://tinybase.org/api/ui-react/functions/state-hooks/usevaluesstate/)(store);
  return (
    <div>
      {JSON.stringify(values)}
      <button onClick={() => setValues[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setvalues/)({...values, employees: 3})}>
        Add
      </button>
    </div>
  );
