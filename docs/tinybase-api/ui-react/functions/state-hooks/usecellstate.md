...
# `useCellState`
The `useCellState` hook returns a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) from a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) and a callback to set it, following the common React `useState` convention.
...
): [CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/), (cell: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)) => void]
```
...
| `storeOrStoreId?`  | `StoreOrStoreId[](https://tinybase.org/api/ui-react/type-aliases/identity/storeorstoreid/)`  | The [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to get data from: omit for the default context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), or provide an explicit reference.  |  
| returns  | `[CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/), (cell: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)) => void]`  | A tuple containing the current [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) and a setter callback that can be called with a new [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) value.  |  
This hook is useful for creating components that read and write a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in a single line, similar to how you would use React's `useState` hook.
The component this is used in will re-render when the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) changes.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) outside the application, which is used in the `useCellState` hook by reference.
...
const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)().setCell[](https://tinybase.org/api/the-essentials/setting-data/setcell/)('pets', 'fido', 'visits', 0);
const App = () => {
  const [visits, setVisits] = useCellState[](https://tinybase.org/api/ui-react/functions/state-hooks/usecellstate/)(
    'pets',
    'fido',
    'visits',
    store,
