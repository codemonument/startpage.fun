...
# `useTableState`
The `useTableState` hook returns a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) and a function to set it, following the same pattern as React's useState hook.

```
useTableState[](https://tinybase.org/api/ui-react/functions/state-hooks/usetablestate/)(
  tableId: string,
  storeOrStoreId?: StoreOrStoreId[](https://tinybase.org/api/ui-react/type-aliases/identity/storeorstoreid/),
): [Table[](https://tinybase.org/api/store/type-aliases/store/table/), (table: Table[](https://tinybase.org/api/store/type-aliases/store/table/)) => void]
```
...
| returns  | `[Table[](https://tinybase.org/api/store/type-aliases/store/table/), (table: Table[](https://tinybase.org/api/store/type-aliases/store/table/)) => void]`  | An array containing the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) and a function to set it.  |  
This is a convenience hook that combines the useTable and [`useSetTableCallback`](https://tinybase.org/api/ui-react/functions/store-hooks/usesettablecallback/) hooks. It's useful when you need both read and write access to a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) in a single component.
A [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) or a set of [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useTableState` hook lets you indicate which [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to use: omit the final parameter for the default context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), or provide a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) explicitly by reference.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) outside the application, which is used in the `useTableState` hook by reference. A button updates the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) when clicked.
...
const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)().setTable[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settable/)('pets', {fido: {species: 'dog'}});
const App = () => {
  const [table, setTable] = useTableState[](https://tinybase.org/api/ui-react/functions/state-hooks/usetablestate/)('pets', store);
  return (
    <div>
      {JSON.stringify(table)}
      <button
        onClick={() => setTable[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settable/)({...table, felix: {species: 'cat'}})}
      >
        Add
      </button>
    </div>
