...
# `useRowState`
The `useRowState` hook returns a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) and a function to set it, following the same pattern as React's useState hook.
...
): [Row[](https://tinybase.org/api/store/type-aliases/store/row/), (row: Row[](https://tinybase.org/api/store/type-aliases/store/row/)) => void]
```
...
| returns  | `[Row[](https://tinybase.org/api/store/type-aliases/store/row/), (row: Row[](https://tinybase.org/api/store/type-aliases/store/row/)) => void]`  | An array containing the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) and a function to set it.  |  
This is a convenience hook that combines the useRow and [`useSetRowCallback`](https://tinybase.org/api/ui-react/functions/store-hooks/usesetrowcallback/) hooks. It's useful when you need both read and write access to a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) in a single component.
A [`Provider`](https://tinybase.org/api/ui-react/functions/context-components/provider/) component is used to wrap part of an application in a context, and it can contain a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) or a set of [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects named by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/). The `useRowState` hook lets you indicate which [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to use: omit the final parameter for the default context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), or provide a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) explicitly by reference.
## Example
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) outside the application, which is used in the `useRowState` hook by reference. A button updates the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) when clicked.
...
const store = createStore[](https://tinybase.org/api/store/functions/creation/createstore/)().setTable[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settable/)('t1', {r1: {c1: 'Alice', c2: 30}});
const App = () => {
  const [row, setRow] = useRowState[](https://tinybase.org/api/ui-react/functions/state-hooks/userowstate/)('t1', 'r1', store);
  return (
    <div>
      {JSON.stringify(row)}
      <button onClick={() => setRow[](https://tinybase.org/api/the-essentials/setting-data/setrow/)({...row, c2: (row.c2 || 0) + 1})}>
        Birthday
      </button>
    </div>
  );
