...
# `CellView`
The `CellView` component renders the value of a single [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in a given [`Row`](https://tinybase.org/api/store/type-aliases/store/row/), in a given [`Table`](https://tinybase.org/api/store/type-aliases/store/table/), and registers a listener so that any changes to that result will cause a re-render.

```
CellView[](https://tinybase.org/api/ui-react/functions/store-components/cellview/)(props: CellProps[](https://tinybase.org/api/ui-react/type-aliases/props/cellprops/)): ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `props`  | `CellProps[](https://tinybase.org/api/ui-react/type-aliases/props/cellprops/)`  | The props for this component.  |  
| returns  | `ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)`  | A rendering of the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/), or nothing, if not present.  |  
The component's props identify which [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) to render based on [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), and [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) (which is either the default context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), a named context [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), or an explicit reference).
A [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) contains a string, number, or boolean, so the value is rendered directly without further decoration. You can create your own CellView-like component to customize the way that a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) is rendered: see the [`RowView`](https://tinybase.org/api/ui-react/functions/store-components/rowview/) component for more details.
This component uses the [`useCell`](https://tinybase.org/api/ui-react/functions/store-hooks/usecell/) hook under the covers, which means that any changes to the specified [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) will cause a re-render.
## Examples
This example creates a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) outside the application, which is used in the `CellView` component by reference. A change to the data in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) re-renders the component.
...
This example creates a Provider context into which a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) is provided. The `CellView` component within it then renders the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) (with its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for readability).
...
const Pane = () => (
  <span>
    <CellView[](https://tinybase.org/api/ui-react/functions/store-components/cellview/) tableId="pets" rowId="fido" cellId="color" debugIds={true} />
  </span>
);
...
This example creates a Provider context into which a default [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) is provided. The `CellView` component within it then attempts to render a non-existent [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/).
...
const Pane = () => (
  <span>
    <CellView[](https://tinybase.org/api/ui-react/functions/store-components/cellview/) tableId="pets" rowId="fido" cellId="height" />
  </span>
);
