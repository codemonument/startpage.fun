...
# `ResultRowView`
The `ResultRowView` component renders the contents of a single [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) in a given query's [`ResultTable`](https://tinybase.org/api/queries/type-aliases/result/resulttable/), and registers a listener so that any changes to that result will cause a re-render.

```
ResultRowView[](https://tinybase.org/api/ui-react/functions/queries-components/resultrowview/)(props: ResultRowProps[](https://tinybase.org/api/ui-react/type-aliases/props/resultrowprops/)): ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `props`  | `ResultRowProps[](https://tinybase.org/api/ui-react/type-aliases/props/resultrowprops/)`  | The props for this component.  |  
| returns  | `ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)`  | A rendering of the result [`Row`](https://tinybase.org/api/store/type-aliases/store/row/), or nothing, if not present.  |  
The component's props identify which [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) to render based on query [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), and [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object (which is either the default context [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, a named context [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object, or an explicit reference).
This component renders a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) by iterating over its [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values. By default these are in turn rendered with the [`ResultCellView`](https://tinybase.org/api/ui-react/functions/queries-components/resultcellview/) component, but you can override this behavior by providing a `resultCellComponent` prop, a custom component of your own that will render a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) based on [`ResultCellProps`](https://tinybase.org/api/ui-react/type-aliases/props/resultcellprops/). You can also pass additional props to your custom component with the `getResultCellComponentProps` callback prop.
You can create your own ResultRowView-like component to customize the way that a result [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) is rendered: see the [`ResultTableView`](https://tinybase.org/api/ui-react/functions/queries-components/resulttableview/) component for more details.
This component uses the [`useResultCellIds`](https://tinybase.org/api/ui-react/functions/queries-hooks/useresultcellids/) hook under the covers, which means that any changes to the structure of the result [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) will cause a re-render.
## Examples
This example creates a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object outside the application, which is used in the `ResultRowView` component by reference. A change to the data in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) re-renders the component.
...
This example creates a Provider context into which a default [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object is provided. The `ResultRowView` component within it then renders the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) (with [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) for readability).
...
const Pane = () => (
  <div>
    <ResultRowView[](https://tinybase.org/api/ui-react/functions/queries-components/resultrowview/) queryId="petColors" rowId="fido" debugIds={true} />
  </div>
);
...
).setQueryDefinition[](https://tinybase.org/api/queries/interfaces/queries/queries/methods/configuration/setquerydefinition/)('petColors', 'pets', ({select}) => {
  select('species');
  select('color');
});
const app = document.createElement('div');
...
This example creates a Provider context into which a default [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object is provided. The `ResultRowView` component within it then renders the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) with a custom [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) component and a custom props callback.
...
const FormattedResultCellView = ({queryId, rowId, cellId, bold}) => (
  <span>
    ...
    {': '}
    <ResultCellView[](https://tinybase.org/api/ui-react/functions/queries-components/resultcellview/) queryId={queryId} rowId={rowId} cellId={cellId} />
  </span>
);
