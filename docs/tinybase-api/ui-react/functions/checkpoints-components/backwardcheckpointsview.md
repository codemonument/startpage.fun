...
The `BackwardCheckpointsView` component renders a list of previous checkpoints that the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) can go back to.

```
BackwardCheckpointsView[](https://tinybase.org/api/ui-react/functions/checkpoints-components/backwardcheckpointsview/)(props: BackwardCheckpointsProps[](https://tinybase.org/api/ui-react/type-aliases/props/backwardcheckpointsprops/)): ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `props`  | `BackwardCheckpointsProps[](https://tinybase.org/api/ui-react/type-aliases/props/backwardcheckpointsprops/)`  | The props for this component.  |  
| returns  | `ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)`  | A rendering of the previous checkpoints, if present.  |  
The component's props identify which previous checkpoints to render based on the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object (which is either the default context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, a named context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, or an explicit reference).
This component renders a list by iterating over each checkpoints. By default these are in turn rendered with the [`CheckpointView`](https://tinybase.org/api/ui-react/functions/checkpoints-components/checkpointview/) component, but you can override this behavior by providing a `checkpointComponent` prop, a custom component of your own that will render a checkpoint based on [`CheckpointProps`](https://tinybase.org/api/ui-react/type-aliases/props/checkpointprops/). You can also pass additional props to your custom component with the `getCheckpointComponentProps` callback prop.
This component uses the [`useCheckpointIds`](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointids/) hook under the covers, which means that any changes to the checkpoint [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object will cause a re-render.
## Examples
This example creates a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object outside the application, which is used in the `BackwardCheckpointsView` component by reference to render a list of previous checkpoints.
...
const store = createStore[](https://tinybase.org/api/the-essentials/creating-stores/createstore/)().setTable[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settable/)('pets', {fido: {color: 'brown'}});
const checkpoints = createCheckpoints[](https://tinybase.org/api/checkpoints/functions/creation/createcheckpoints/)(store);
const App = () => (
  <div>
    <BackwardCheckpointsView[](https://tinybase.org/api/ui-react/functions/checkpoints-components/backwardcheckpointsview/) checkpoints={checkpoints} separator="/" />
  </div>
);
...
checkpoints.setCheckpoint[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/setter/setcheckpoint/)('0', 'initial');
store.setCell[](https://tinybase.org/api/the-essentials/setting-data/setcell/)('pets', 'fido', 'species', 'dog');
checkpoints.addCheckpoint[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/setter/addcheckpoint/)('identified');
console.log(app.innerHTML);
// -> '<div>initial</div>'
...
This example creates a Provider context into which a default [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object is provided. The `BackwardCheckpointsView` component within it then renders the list of previous checkpoints (with [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) for readability).
...
const Pane = () => (
  <div>
    <BackwardCheckpointsView[](https://tinybase.org/api/ui-react/functions/checkpoints-components/backwardcheckpointsview/) debugIds={true} />
  </div>
);
...
This example creates a Provider context into which a default [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object is provided. The `BackwardCheckpointsView` component within it then renders the list of previous checkpoints with a custom [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) component and a custom props callback.
