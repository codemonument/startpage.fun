...
# `LinkedRowsView`
The `LinkedRowsView` component renders the local [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects for a given remote [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) in a [`Relationship`](https://tinybase.org/api/relationships/type-aliases/concept/relationship/), and registers a listener so that any changes to that result will cause a re-render.

```
LinkedRowsView[](https://tinybase.org/api/ui-react/functions/relationships-components/linkedrowsview/)(props: LinkedRowsProps[](https://tinybase.org/api/ui-react/type-aliases/props/linkedrowsprops/)): ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `props`  | `LinkedRowsProps[](https://tinybase.org/api/ui-react/type-aliases/props/linkedrowsprops/)`  | The props for this component.  |  
| returns  | `ComponentReturnType[](https://tinybase.org/api/ui-react/type-aliases/component/componentreturntype/)`  | A rendering of the local [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects, or nothing, if not present.  |  
The component's props identify which local Rows to render based on [`Relationship`](https://tinybase.org/api/relationships/type-aliases/concept/relationship/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), remote [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), and [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object (which is either the default context [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object, a named context [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object, or an explicit reference).
By default the local Rows are rendered with the [`RowView`](https://tinybase.org/api/ui-react/functions/store-components/rowview/) component, but you can override this behavior by providing a `rowComponent` prop, a custom component of your own that will render the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) based on [`RowProps`](https://tinybase.org/api/ui-react/type-aliases/props/rowprops/). You can also pass additional props to your custom component with the `getRowComponentProps` callback prop.
This component uses the [`useLocalRowIds`](https://tinybase.org/api/ui-react/functions/relationships-hooks/uselocalrowids/) hook under the covers, which means that any changes to the local [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) in the [`Relationship`](https://tinybase.org/api/relationships/type-aliases/concept/relationship/) will cause a re-render.
## Examples
This example creates a [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object outside the application, which is used in the `LinkedRowsView` component by reference. A change to the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) re-renders the component.
...
store.setRow[](https://tinybase.org/api/the-essentials/setting-data/setrow/)('pets', 'cujo', {next: 'toto'});
console.log(app.innerHTML);
// -> '<div>felix/cujo/toto/dog</div>'

```

This example creates a Provider context into which a default [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object is provided. The `LinkedRowsView` component within it then renders the local [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects (with [`Ids`](https://tinybase.org/api/common/type-aliases/identity/ids/) for readability).
...
This example creates a Provider context into which a default [`Relationships`](https://tinybase.org/api/relationships/interfaces/relationships/relationships/) object is provided. The `LinkedRowsView` component within it then renders the local [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects with a custom [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) component and a custom props callback.
...
const FormattedRowView = ({store, tableId, rowId, bold}) => (
  <span>
    ...
    {': '}
    <RowView[](https://tinybase.org/api/ui-react/functions/store-components/rowview/) store={store} tableId={tableId} rowId={rowId} />
  </span>
);
