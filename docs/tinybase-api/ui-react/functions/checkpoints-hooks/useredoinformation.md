...
# `useRedoInformation`
The `useRedoInformation` hook returns an [`UndoOrRedoInformation`](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/) array that indicates if and how you can move the state of the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) forwards to a future checkpoint.

```
useRedoInformation[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/useredoinformation/)(checkpointsOrCheckpointsId?: CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-react/type-aliases/identity/checkpointsorcheckpointsid/)): UndoOrRedoInformation[](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `checkpointsOrCheckpointsId?`  | `CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-react/type-aliases/identity/checkpointsorcheckpointsid/)`  | The [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to use to go backward: omit for the default context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, or provide an explicit reference.  |  
| returns  | `UndoOrRedoInformation[](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/)`  | [`UndoOrRedoInformation`](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/) about if and how you can move the state of the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) forward.  |  
This hook is useful if you are building an redo button: the information contains whether a redo action is available (to enable the button), the callback to perform the redo action, the checkpoint [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) that will be redone, and its label, if available.
## Example
This example uses the [`useUndoInformation`](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/useundoinformation/) hook to create a redo button.
...
const App = () => {
  const [canRedo, handleRedo, _id, label] =
    useRedoInformation[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/useredoinformation/)(checkpoints);
  return canRedo ? (
...
checkpoints.goTo[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/movement/goto/)('0');
console.log(app.innerHTML);
