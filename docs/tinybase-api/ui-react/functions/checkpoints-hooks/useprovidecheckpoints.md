...
The `useProvideCheckpoints` hook is used to add a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) to a [`Provider`](https://tinybase.org/api/ui-react/functions/context-components/provider/) component, but imperatively from a component within it.
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `checkpointsId`  | `string`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to be registered with the Provider.  |  
| `checkpoints`  | `Checkpoints[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/)`  | The [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to be registered.  |  
| returns  | `void`  | This has no return value.  |  
Normally you will register a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in a context by using the `checkpointsById` prop of the top-level [`Provider`](https://tinybase.org/api/ui-react/functions/context-components/provider/) component. This hook, however, lets you dynamically add a new [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to the context, from within a component. This is useful for applications where the set of [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) objects is not known at the time of the first render of the root Provider.
A [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object added to the Provider context in this way will be available to other components within the context (using the useCheckpoints hook and so on). If you use the same [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) as an existing [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object registration, the new one will take priority over one provided by the `checkpointsById` prop.
Note that other components that consume a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object registered like this should defend against it being undefined at first. On the first render, the other component will likely not yet have completed the registration. In the example below, we use the null-safe `useCheckpoints('petCheckpoints')?` to do this.
## Example
This example creates a Provider context. A child component registers a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object into it which is then consumable by a peer child component.
...
const RegisterCheckpoints = () => {
  const store = useCreateStore[](https://tinybase.org/api/ui-react/functions/store-hooks/usecreatestore/)(() =>
    createStore[](https://tinybase.org/api/the-essentials/creating-stores/createstore/)().setCell[](https://tinybase.org/api/the-essentials/setting-data/setcell/)('pets', 'fido', 'color', 'brown'),
  );
  ...
  useProvideCheckpoints[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/useprovidecheckpoints/)('petCheckpoints', checkpoints);
  return null;
};
const ConsumeCheckpoints = () => (
  <span>
    {JSON.stringify(useCheckpoints[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpoints/)('petCheckpoints')?.getCheckpointIds[](https://tinybase.org/api/ui-svelte/functions/getter/getcheckpointids/)())}
  </span>
);
