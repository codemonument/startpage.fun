...
The `useCheckpointIdsListener` hook registers a listener function with the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object that will be called whenever its set of checkpoints changes.

```
useCheckpointIdsListener[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointidslistener/)(
  listener: CheckpointIdsListener[](https://tinybase.org/api/checkpoints/type-aliases/listener/checkpointidslistener/),
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `listener`  | `CheckpointIdsListener[](https://tinybase.org/api/checkpoints/type-aliases/listener/checkpointidslistener/)`  | The function that will be called whenever the checkpoints change.  |  
...
| `checkpointsOrCheckpointsId?`  | `CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-react/type-aliases/identity/checkpointsorcheckpointsid/)`  | The [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to register the listener with: omit for the default context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, provide an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for a named context [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object, or provide an explicit reference.  |  
| returns  | `void`  | This has no return value.  |  
This hook is useful for situations where a component needs to register its own specific listener to do more than simply tracking the value (which is more easily done with the [`useCheckpointIds`](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointids/) hook).
Unlike the [`addCheckpointIdsListener`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/listener/addcheckpointidslistener/) method, which returns a listener [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and requires you to remove it manually, the `useCheckpointIdsListener` hook manages this lifecycle for you: when the listener changes (per its `listenerDeps` dependencies) or the component unmounts, the listener on the underlying [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object will be deleted.
## Example
This example uses the `useCheckpointIdsListener` hook to create a listener that is scoped to a single component. When the component is unmounted, the listener is removed from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
const Pane = () => {
  useCheckpointIdsListener[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointidslistener/)(() => console.log('Checkpoint Ids changed'));
...
console.log(checkpoints.getListenerStats[](https://tinybase.org/api/store/interfaces/store/store/methods/development/getlistenerstats/)().checkpointIds);
// -> 1
