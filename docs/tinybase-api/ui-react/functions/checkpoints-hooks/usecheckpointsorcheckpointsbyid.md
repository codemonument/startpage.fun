...
The `useCheckpointsOrCheckpointsById` hook is used to get a reference to a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object from within a [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component context, _or_ have it passed directly to this hook.

```
useCheckpointsOrCheckpointsById[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointsorcheckpointsbyid/)(checkpointsOrCheckpointsId?: CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-react/type-aliases/identity/checkpointsorcheckpointsid/)): Checkpoints[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) | undefined
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `checkpointsOrCheckpointsId?`  | `CheckpointsOrCheckpointsId[](https://tinybase.org/api/ui-react/type-aliases/identity/checkpointsorcheckpointsid/)`  | Either an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) for accessing a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object that was named with an [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in the Provider, or the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object itself.  |  
...
This is mostly of use when you are developing a component that needs a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object and which might have been passed in explicitly to the component or is to be picked up from the context by [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) (a common pattern for [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/)-based components).
This is unlikely to be used often. For most situations, you will want to use the [`useCheckpoints`](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpoints/) hook.
## Example
This example creates a Provider context into which a default [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object is provided. A component within it then uses the `useCheckpointsOrCheckpointsById` hook to get a reference to the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object again, without the need to have it passed as a prop. Note however, that unlike the [`useCheckpoints`](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpoints/) hook example, this component would also work if you were to pass the [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object directly into it, making it more portable.
...
const Pane = ({checkpoints}) => (
  <span>
    {JSON.stringify(
      useCheckpointsOrCheckpointsById[](https://tinybase.org/api/ui-react/functions/checkpoints-hooks/usecheckpointsorcheckpointsbyid/)(checkpoints).getCheckpointIds[](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/methods/getter/getcheckpointids/)(),
    )}
  </span>
);
