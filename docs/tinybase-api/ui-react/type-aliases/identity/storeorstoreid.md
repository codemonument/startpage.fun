...
# `StoreOrStoreId`
The `StoreOrStoreId` type is used when you need to refer to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) in a React hook or component.

```
Store[](https://tinybase.org/api/the-essentials/creating-stores/store/) | Id[](https://tinybase.org/api/common/type-aliases/identity/id/)
```

In some simple cases you will already have a direct reference to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
This module also includes a [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component that can be used to wrap multiple [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) objects into a context that can be used throughout the app. In this case you will want to refer to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) by its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in that context.
Many hooks and components in this [`ui-react`](https://tinybase.org/api/ui-react/) module take this type as a parameter or a prop, allowing you to pass in either the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) or its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Since
v1.0.0
