...
# `QueriesOrQueriesId`
The `QueriesOrQueriesId` type is used when you need to refer to a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object in a React hook or component.

```
Queries[](https://tinybase.org/api/queries/interfaces/queries/queries/) | Id[](https://tinybase.org/api/common/type-aliases/identity/id/)
```

In some simple cases you will already have a direct reference to the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object.
This module also includes a [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component that can be used to wrap multiple [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) objects into a context that can be used throughout the app. In this case you will want to refer to a [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object by its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in that context.
Many hooks and components in this [`ui-react`](https://tinybase.org/api/ui-react/) module take this type as a parameter or a prop, allowing you to pass in either the [`Queries`](https://tinybase.org/api/queries/interfaces/queries/queries/) object or its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Since
v2.0.0
