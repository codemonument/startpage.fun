...
# `IndexesOrIndexesId`
The `IndexesOrIndexesId` type is used when you need to refer to an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object in a React hook or component.

```
Indexes[](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) | Id[](https://tinybase.org/api/common/type-aliases/identity/id/)
```

In some simple cases you will already have a direct reference to the [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object.
This module also includes a [`Provider`](https://tinybase.org/api/the-essentials/using-react/provider/) component that can be used to wrap multiple [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) objects into a context that can be used throughout the app. In this case you will want to refer to an [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object by its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) in that context.
Many hooks and components in this [`ui-react`](https://tinybase.org/api/ui-react/) module take this type as a parameter or a prop, allowing you to pass in either the [`Indexes`](https://tinybase.org/api/indexes/interfaces/indexes/indexes/) object or its [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Since
v1.0.0
