...
The [`UndoOrRedoInformation`](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/) type is an array that you can fetch from a [`Checkpoints`](https://tinybase.org/api/checkpoints/interfaces/checkpoints/checkpoints/) object to that indicates if and how you can move the state of the underlying [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) forward or backward. [Read more](https://tinybase.org/api/ui-react/type-aliases/checkpoints/undoorredoinformation/).
