...
# `persister-browser`
The `persister-browser` module of the TinyBase project lets you save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from browser storage, including the origin private file system (OPFS).
Three entry points are provided, each of which returns a new [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) object that can load and save a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/):
  * The [`createSessionPersister`](https://tinybase.org/api/persister-browser/functions/creation/createsessionpersister/) function returns a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) that uses the browser's session storage.
  * The [`createLocalPersister`](https://tinybase.org/api/persister-browser/functions/creation/createlocalpersister/) function returns a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) that uses the browser's local storage.
  * The [`createOpfsPersister`](https://tinybase.org/api/persister-browser/functions/creation/createopfspersister/) function returns a [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) that uses a file in an origin private file system (OPFS).
