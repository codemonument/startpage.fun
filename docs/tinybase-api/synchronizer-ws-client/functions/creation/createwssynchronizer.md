...
Essential
The `createWsSynchronizer` function creates a [`WsSynchronizer`](https://tinybase.org/api/synchronizer-ws-client/interfaces/synchronizer/wssynchronizer/) object that can synchronize [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) data to and from other [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) instances via WebSockets facilitated by a [`WsServer`](https://tinybase.org/api/synchronizer-ws-server/interfaces/server/wsserver/).

```
createWsSynchronizer[](https://tinybase.org/api/synchronizer-ws-client/functions/creation/createwssynchronizer/)<WebSocketType>(
...
| `requestTimeoutSeconds?`  | `number`  | An optional time in seconds that the [`Synchronizer`](https://tinybase.org/api/synchronizers/interfaces/synchronizer/synchronizer/) will wait for responses to request messages, defaulting to 1.  |  
...
| returns  | `Promise<WsSynchronizer[](https://tinybase.org/api/synchronizer-ws-client/interfaces/synchronizer/wssynchronizer/)<WebSocketType>>`  | A reference to the new [`WsSynchronizer`](https://tinybase.org/api/synchronizer-ws-client/interfaces/synchronizer/wssynchronizer/) object.  |  
As well as providing a reference to the [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) to persist, you must provide a configured WebSocket to send synchronization messages over.
Instead of the raw browser implementation of WebSocket, you may prefer to use the [Reconnecting WebSocket](https://github.com/pladaria/reconnecting-websocket) wrapper so that if a client goes offline, it can easily re-establish a connection when it comes back online. Its [API](https://tinybase.org/api/) is compatible with this [`Synchronizer`](https://tinybase.org/api/synchronizers/interfaces/synchronizer/synchronizer/).
You can indicate how long the [`Synchronizer`](https://tinybase.org/api/synchronizers/interfaces/synchronizer/synchronizer/) will wait for responses to message requests before timing out. A final set of optional handlers can be provided to help debug sends, receives, and errors respectively.
...
## Example
This example creates two [`WsSynchronizer`](https://tinybase.org/api/synchronizer-ws-client/interfaces/synchronizer/wssynchronizer/) objects to synchronize one [`MergeableStore`](https://tinybase.org/api/mergeable-store/interfaces/mergeable/mergeablestore/) to another via a server.
...
await server.destroy[](https://tinybase.org/api/synchronizer-ws-client/interfaces/synchronizer/wssynchronizer/methods/lifecycle/destroy/)();
