...
# `getTransactionLog`
The `getTransactionLog` method returns the changes that were made to a [`Store`](https://tinybase.org/api/store/interfaces/store/store/) during a transaction in more detail, including invalid changes, and what previous values were.

```
getTransactionLog[](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/gettransactionlog/)(): TransactionLog[](https://tinybase.org/api/store/type-aliases/transaction/transactionlog/)
```
...
| --- | --- | --- |  
This is useful for deciding whether to rollback a transaction, for example. The returned object is only meaningful if the method is called when the [`Store`](https://tinybase.org/api/store/interfaces/store/store/) is in a transaction - such as in a [`TransactionListener`](https://tinybase.org/api/store/type-aliases/listener/transactionlistener/).
## Example
This example makes changes to the [`Store`](https://tinybase.org/api/store/interfaces/store/store/). At the end of the transaction, detail about what changed is enumerated.
