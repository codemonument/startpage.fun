        ...
        * [Interfaces](https://tinybase.org/api/store/interfaces/)
          * [`Store`](https://tinybase.org/api/store/interfaces/store/store/)
            ...
            * [Transaction methods](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/)
              * [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/)
              * [`getTransactionChanges`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/gettransactionchanges/)
              * [`getTransactionLog`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/gettransactionlog/)
              * [`startTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/starttransaction/)
              * [`transaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/transaction/)
...
# `startTransaction`
The `startTransaction` method allows you to explicitly start a transaction that will make multiple mutations to the [`Store`](https://tinybase.org/api/store/interfaces/store/store/), buffering all calls to the relevant listeners until it completes when you call the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method.

```
startTransaction[](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/starttransaction/)(): this
```
  
| returns  | `this`  | A reference to the [`Store`](https://tinybase.org/api/store/interfaces/store/store/).  |  
| --- | --- | --- |  
[Transactions](https://tinybase.org/guides/the-basics/transactions/) are useful for making bulk changes to the data in a [`Store`](https://tinybase.org/api/store/interfaces/store/store/), and when you don't want listeners to be called as you make each change. [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/) are made silently during the transaction, and listeners relevant to the changes you have made will instead only be called when the whole transaction is complete.
Generally it is preferable to use the [`transaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/transaction/) method to wrap a block of code as a transaction. It simply calls both the startTransaction and [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) methods for you. See that method for several transaction examples.
Use this `startTransaction` method when you have a more 'open-ended' transaction, such as one containing mutations triggered from other events that are asynchronous or not occurring inline to your code. You must remember to also call the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method explicitly when it is done, of course.
## Example
...
store.addRowListener[](https://tinybase.org/api/store/interfaces/store/store/methods/listener/addrowlistener/)('pets', 'fido', () => console.log('Fido changed'));

store
  .setCell[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcell/)('pets', 'fido', 'color', 'brown')
  .setCell[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcell/)('pets', 'fido', 'sold', false);
// -> 'Fido changed'
// -> 'Fido changed'

store
  .startTransaction[](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/starttransaction/)()
  .setCell[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcell/)('pets', 'fido', 'color', 'walnut')
  .setCell[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setcell/)('pets', 'fido', 'sold', true)
  .finishTransaction[](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/)();
// -> 'Fido changed'
