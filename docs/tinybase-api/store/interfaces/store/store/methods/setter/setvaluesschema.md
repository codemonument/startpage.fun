...
# `setValuesSchema`
The `setValuesSchema` method lets you specify the [`ValuesSchema`](https://tinybase.org/api/store/type-aliases/schema/valuesschema/) of the keyed [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) part of the [`Store`](https://tinybase.org/api/store/interfaces/store/store/).

```
setValuesSchema[](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setvaluesschema/)(valuesSchema: ValuesSchema[](https://tinybase.org/api/store/type-aliases/schema/valuesschema/)): this
```
...
Note that this may result in a change to data in the [`Store`](https://tinybase.org/api/store/interfaces/store/store/), as defaults are applied or as invalid [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) are removed. These changes will fire any listeners to that data, as expected.
When no longer needed, you can also completely remove an existing [`ValuesSchema`](https://tinybase.org/api/store/type-aliases/schema/valuesschema/) with the [`delValuesSchema`](https://tinybase.org/api/store/interfaces/store/store/methods/deleter/delvaluesschema/) method.
## Example
This example sets the [`ValuesSchema`](https://tinybase.org/api/store/type-aliases/schema/valuesschema/) of a [`Store`](https://tinybase.org/api/store/interfaces/store/store/) after it has been created.
