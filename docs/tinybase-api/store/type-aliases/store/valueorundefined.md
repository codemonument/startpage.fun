...
```
Value[](https://tinybase.org/api/store/type-aliases/store/value/) | undefined
```

This is used when describing a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) that is present _or_ that is not present, such as when it has been deleted, or when describing a previous state where the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) has since been added.
## Since
v3.0.0
