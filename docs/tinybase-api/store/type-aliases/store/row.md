...
```
{[cellId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)}
```

A `Row` is used when setting a row with the [`setRow`](https://tinybase.org/api/the-essentials/setting-data/setrow/) method, and when getting it back out again with the [`getRow`](https://tinybase.org/api/the-essentials/getting-data/getrow/) method. A `Row` object is a regular JavaScript object containing individual [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) objects, keyed by their [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Example
