...
This is used when describing a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) that is present _or_ that is not present, such as when it has been deleted, or when describing a previous state where the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) value has since been added.
## Since
v1.0.0
