...
# `Tables`
The `Tables` type is the data structure representing all of the data in a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
{[tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: Table[](https://tinybase.org/api/store/type-aliases/store/table/)}
```

A `Tables` object is used when setting all of the tables together with the [`setTables`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settables/) method, and when getting them back out again with the [`getTables`](https://tinybase.org/api/store/interfaces/store/store/methods/getter/gettables/) method. A `Tables` object is a regular JavaScript object containing individual [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) objects, keyed by their [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Example
