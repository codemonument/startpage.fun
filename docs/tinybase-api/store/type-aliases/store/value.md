...
```
string | number | boolean | null | AnyObject[](https://tinybase.org/api/common/type-aliases/general/anyobject/) | AnyArray[](https://tinybase.org/api/common/type-aliases/general/anyarray/)
```

A `Value` is used when setting a value with the [`setValue`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setvalue/) method, and when getting it back out again with the [`getValue`](https://tinybase.org/api/the-essentials/getting-data/getvalue/) method. A `Value` is a JavaScript string, number, boolean, or null (since v7.0), or a plain JavaScript object or array (since v8.0).
## Example
