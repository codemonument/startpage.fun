...
# `Values`
The `Values` type is the data structure representing all the keyed values in a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).

```
{[valueId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: Value[](https://tinybase.org/api/store/type-aliases/store/value/)}
```

A `Values` object is used when setting values with the [`setValues`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setvalues/) method, and when getting them back out again with the [`getValues`](https://tinybase.org/api/store/interfaces/store/store/methods/getter/getvalues/) method. A `Values` object is a regular JavaScript object containing individual [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) objects, keyed by their [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Example
