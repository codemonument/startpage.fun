...
```
{[rowId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: Row[](https://tinybase.org/api/store/type-aliases/store/row/)}
```

A `Table` is used when setting a table with the [`setTable`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/settable/) method, and when getting it back out again with the [`getTable`](https://tinybase.org/api/store/interfaces/store/store/methods/getter/gettable/) method. A `Table` object is a regular JavaScript object containing individual [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects, keyed by their [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/).
## Example
