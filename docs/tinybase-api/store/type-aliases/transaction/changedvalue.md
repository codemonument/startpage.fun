...
# `ChangedValue`
The `ChangedValue` type describes a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) that has been changed during a transaction, primarily used so that you can indicate whether the transaction should be rolled back.

```
[oldValue: ValueOrUndefined[](https://tinybase.org/api/store/type-aliases/store/valueorundefined/), newValue: ValueOrUndefined[](https://tinybase.org/api/store/type-aliases/store/valueorundefined/)]
```

It provides both the old and new [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) in a two-part array. These describe the state of the changed [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) at the _start_ of the transaction, and by the _end_ of the transaction.
Hence, an `undefined` value for the first item in the array means that the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) was added during the transaction. An `undefined` value for the second item in the array means that the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) was removed during the transaction. An array with two different [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) indicates that it was changed. The two-part array will never contain two items of the same value (including two `undefined` values), even if, during the transaction, a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) was changed to a different value and then changed back.
## Since
v3.0.0
