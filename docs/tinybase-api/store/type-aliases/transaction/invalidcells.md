...
# `InvalidCells`
The `InvalidCells` type describes the invalid [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values that have been attempted during a transaction, primarily used so that you can indicate whether the transaction should be rolled back.

```
{[tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: {[rowId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: {[cellId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: any[]}}}
```

An `InvalidCells` object is provided to the `doRollback` callback when using the [`transaction`](https://tinybase.org/api/the-essentials/setting-data/transaction/) method and the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method. See those methods for specific examples.
This type is a nested structure of [`Table`](https://tinybase.org/api/store/type-aliases/store/table/), [`Row`](https://tinybase.org/api/store/type-aliases/store/row/), and [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) objects, much like the [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) object, but one for which [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values are listed in array (much like the [`InvalidCellListener`](https://tinybase.org/api/store/type-aliases/listener/invalidcelllistener/) type) so that multiple failed attempts to change a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) during the transaction are described.
## Since
v1.2.0
