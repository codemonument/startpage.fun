      ...
      * [`store`](https://tinybase.org/api/store/)
        ...
        * [Type Aliases](https://tinybase.org/api/store/type-aliases/)
          ...
          * [Transaction type aliases](https://tinybase.org/api/store/type-aliases/transaction/)
            * [`ChangedTableIds`](https://tinybase.org/api/store/type-aliases/transaction/changedtableids/)
            * [`ChangedRowIds`](https://tinybase.org/api/store/type-aliases/transaction/changedrowids/)
            * [`ChangedCellIds`](https://tinybase.org/api/store/type-aliases/transaction/changedcellids/)
            * [`ChangedCell`](https://tinybase.org/api/store/type-aliases/transaction/changedcell/)
            * [`ChangedCells`](https://tinybase.org/api/store/type-aliases/transaction/changedcells/)
            * [`InvalidCells`](https://tinybase.org/api/store/type-aliases/transaction/invalidcells/)
            * [`ChangedValues`](https://tinybase.org/api/store/type-aliases/transaction/changedvalues/)
            * [`InvalidValues`](https://tinybase.org/api/store/type-aliases/transaction/invalidvalues/)
            * [`ChangedValue`](https://tinybase.org/api/store/type-aliases/transaction/changedvalue/)
            * [`ChangedValueIds`](https://tinybase.org/api/store/type-aliases/transaction/changedvalueids/)
            * [`Changes`](https://tinybase.org/api/store/type-aliases/transaction/changes/)
            * [`IdAddedOrRemoved`](https://tinybase.org/api/store/type-aliases/transaction/idaddedorremoved/)
            * [`TransactionLog`](https://tinybase.org/api/store/type-aliases/transaction/transactionlog/)
...
# `ChangedCells`
The `ChangedCells` type describes the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values that have been changed during a transaction, primarily used so that you can indicate whether the transaction should be rolled back.

```
{[tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: {[rowId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: {[cellId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: ChangedCell[](https://tinybase.org/api/store/type-aliases/transaction/changedcell/)}}}
```

A `ChangedCells` object is provided to the `doRollback` callback when using the [`transaction`](https://tinybase.org/api/the-essentials/setting-data/transaction/) method and the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method. See those methods for specific examples.
This type is a nested structure of [`Table`](https://tinybase.org/api/store/type-aliases/store/table/), [`Row`](https://tinybase.org/api/store/type-aliases/store/row/), and [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) objects, much like the [`Tables`](https://tinybase.org/api/store/type-aliases/store/tables/) object, but one which provides both the old and new [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values in a two-part array. These are describing the state of each changed [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) at the _start_ of the transaction, and by the _end_ of the transaction.
Hence, an `undefined` value for the first item in the array means that the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) was added during the transaction. An `undefined` value for the second item in the array means that the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) was removed during the transaction. An array with two different [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values indicates that it was changed. The two-part array will never contain two items of the same value (including two `undefined` values), even if, during the transaction, a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) was changed to a different value and then changed back.
## Since
v1.2.0
