...
# `InvalidValues`
The `InvalidValues` type describes the invalid [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) that have been attempted during a transaction, primarily used so that you can indicate whether the transaction should be rolled back.

```
{[valueId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: any[]}
```

An `InvalidValues` object is provided to the `doRollback` callback when using the [`transaction`](https://tinybase.org/api/the-essentials/setting-data/transaction/) method and the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method. See those methods for specific examples.
This type is an object containing each invalid [`Value`](https://tinybase.org/api/store/type-aliases/store/value/)'s attempt listed in array (much like the [`InvalidValueListener`](https://tinybase.org/api/store/type-aliases/listener/invalidvaluelistener/) type) so that multiple failed attempts to change a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) during the transaction are described.
## Since
v3.0.0
