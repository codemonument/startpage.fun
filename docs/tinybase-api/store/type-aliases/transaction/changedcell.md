...
# `ChangedCell`
The `ChangedCell` type describes a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) that has been changed during a transaction, primarily used so that you can indicate whether the transaction should be rolled back.

```
[oldCell: CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/), newCell: CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/)]
```

It provides both the old and new [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values in a two-part array. These are describing the state of the changed [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) at the _start_ of the transaction, and by the _end_ of the transaction.
