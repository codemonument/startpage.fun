...
```
(store: Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)): boolean
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `store`  | `Store[](https://tinybase.org/api/the-essentials/creating-stores/store/)`  | A reference to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) that is completing a transaction.  |  
| returns  | `boolean`  |   |  
A `DoRollback` can be provided when calling the [`transaction`](https://tinybase.org/api/the-essentials/setting-data/transaction/) method or the [`finishTransaction`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/finishtransaction/) method. See those methods for specific examples.
Since v5.0, this function is called with the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) as a single argument. You can use the [`getTransactionChanges`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/gettransactionchanges/) method and [`getTransactionLog`](https://tinybase.org/api/store/interfaces/store/store/methods/transaction/gettransactionlog/) method of the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) directly to decide whether to do the rollback.
## Since
v1.0.0
