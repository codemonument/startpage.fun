...
# `ValueCallback`
The `ValueCallback` type describes a function that takes a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/)'s [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and its actual value.
...
|   | Type  | Description  |  
| --- | --- | --- |  
| `valueId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) that the callback can operate on.  |  
| `value`  | `Value[](https://tinybase.org/api/store/type-aliases/store/value/)`  | The [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) itself.  |  
| returns  | `void`  | This has no return value.  |  
A `ValueCallback` is provided when using the [`forEachValue`](https://tinybase.org/api/store/interfaces/store/store/methods/iterator/foreachvalue/) method, so that you can do something based on every [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) in a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/). See that method for specific examples.
## Since
v3.0.0
