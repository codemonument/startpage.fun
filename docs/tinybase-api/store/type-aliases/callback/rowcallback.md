...
# `RowCallback`
The `RowCallback` type describes a function that takes a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/)'s [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and a callback to loop over each [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) within it.

```
(
  rowId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/),
  forEachCell: (cellCallback: CellCallback[](https://tinybase.org/api/store/type-aliases/callback/cellcallback/)) => void,
): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `rowId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) that the callback can operate on.  |  
| `forEachCell`  | `(cellCallback: CellCallback[](https://tinybase.org/api/store/type-aliases/callback/cellcallback/)) => void`  | A function that will let you iterate over the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) values in this [`Row`](https://tinybase.org/api/store/type-aliases/store/row/).  |  
| returns  | `void`  | This has no return value.  |  
A `RowCallback` is provided when using the [`forEachRow`](https://tinybase.org/api/store/interfaces/store/store/methods/iterator/foreachrow/) method, so that you can do something based on every [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) in a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/). See that method for specific examples.
## Since
v1.0.0
