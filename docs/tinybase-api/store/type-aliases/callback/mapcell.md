...
# `MapCell`
The `MapCell` type describes a function that takes an existing [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) value and returns another.

```
(cell: CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/)): Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `cell`  | `CellOrUndefined[](https://tinybase.org/api/store/type-aliases/store/cellorundefined/)`  | The current value of the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) to map to a new value.  |  
| returns  | `Cell[](https://tinybase.org/api/store/type-aliases/store/cell/)`  |   |  
A `MapCell` can be provided in the [`setCell`](https://tinybase.org/api/the-essentials/setting-data/setcell/) method to map an existing value to a new one, such as when incrementing a number. See that method for specific examples.
## Since
v1.0.0
