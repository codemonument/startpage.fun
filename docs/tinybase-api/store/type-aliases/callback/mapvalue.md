...
# `MapValue`
The `MapValue` type describes a function that takes an existing [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) and returns another.

```
(value: ValueOrUndefined[](https://tinybase.org/api/store/type-aliases/store/valueorundefined/)): Value[](https://tinybase.org/api/store/type-aliases/store/value/)
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `value`  | `ValueOrUndefined[](https://tinybase.org/api/store/type-aliases/store/valueorundefined/)`  | The current [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) to map to a new [`Value`](https://tinybase.org/api/store/type-aliases/store/value/).  |  
| returns  | `Value[](https://tinybase.org/api/store/type-aliases/store/value/)`  |   |  
A `MapValue` can be provided in the [`setValue`](https://tinybase.org/api/the-essentials/setting-data/setvalue/) method to map an existing [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) to a new one, such as when incrementing a number. See that method for specific examples.
## Since
v3.0.0
