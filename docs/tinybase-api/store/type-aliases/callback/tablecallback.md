...
# `TableCallback`
The `TableCallback` type describes a function that takes a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/)'s [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) and a callback to loop over each [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) within it.

```
(
  tableId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/),
  forEachRow: (rowCallback: RowCallback[](https://tinybase.org/api/store/type-aliases/callback/rowcallback/)) => void,
): void
```
  
|   | Type  | Description  |  
| --- | --- | --- |  
| `tableId`  | `Id[](https://tinybase.org/api/common/type-aliases/identity/id/)`  | The [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) of the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) that the callback can operate on.  |  
| `forEachRow`  | `(rowCallback: RowCallback[](https://tinybase.org/api/store/type-aliases/callback/rowcallback/)) => void`  | A function that will let you iterate over the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) objects in this [`Table`](https://tinybase.org/api/store/type-aliases/store/table/).  |  
| returns  | `void`  | This has no return value.  |  
A `TableCallback` is provided when using the [`forEachTable`](https://tinybase.org/api/store/interfaces/store/store/methods/iterator/foreachtable/) method, so that you can do something based on every [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) in the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/). See that method for specific examples.
## Since
v1.0.0
