...
# `NoValuesSchema`
The `NoValuesSchema` type is a [`ValuesSchema`](https://tinybase.org/api/store/type-aliases/schema/valuesschema/)-like type for when one has not been provided.

```
{[valueId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: {type: "any"}}
```
