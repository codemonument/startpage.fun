...
# `ValuesSchema`
The `ValuesSchema` type describes the keyed [`Values`](https://tinybase.org/api/store/type-aliases/store/values/) that can be set in a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) and their types.

```
{[valueId: Id[](https://tinybase.org/api/common/type-aliases/identity/id/)]: ValueSchema[](https://tinybase.org/api/store/type-aliases/schema/valueschema/)}
```

A `ValuesSchema` comprises a JavaScript object describing each [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) and its [`ValueSchema`](https://tinybase.org/api/store/type-aliases/schema/valueschema/). It is provided to the [`setValuesSchema`](https://tinybase.org/api/store/interfaces/store/store/methods/setter/setvaluesschema/) method.
## Example
When applied to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), this `ValuesSchema` only allows one boolean [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) called `open`, that defaults to `false`.
