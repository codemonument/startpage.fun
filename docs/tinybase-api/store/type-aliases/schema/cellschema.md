...
# `CellSchema`
The `CellSchema` type describes what values are allowed for each [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) in a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/).
...
```
{
  ...
  type: "object";
  default?: AnyObject[](https://tinybase.org/api/common/type-aliases/general/anyobject/);
  allowNull?: boolean;
} | {
  type: "array";
  default?: AnyArray[](https://tinybase.org/api/common/type-aliases/general/anyarray/);
  allowNull?: boolean;
}
```

A `CellSchema` specifies the type of the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) (`string`, `boolean`, `number`, `null` since v7.0, or `object` or `array` since v8.0), and what the default value can be when an explicit value is not specified.
For `object` and `array` types, TinyBase automatically serializes values to and from JSON when storing and retrieving them. This means you can set an object or array [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) directly, and get back the same structure.
If a default value is provided (and its type is correct), you can be certain that that [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) will always be present in a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/).
If the default value is _not_ provided (or its type is incorrect), the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) may be missing from the [`Row`](https://tinybase.org/api/store/type-aliases/store/row/), but when present you can be guaranteed it is of the correct type.
## Examples
When applied to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), this `CellSchema` ensures a boolean [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) is always present, and defaults it to `false`.
...
When applied to a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), this `CellSchema` allows an object [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) containing arbitrary data, defaulting to an empty object.
