...
# `startAutoSave`
The [`save`](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/methods/save/save/) method takes data from the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) with which the [`Persister`](https://tinybase.org/api/persisters/interfaces/persister/persister/) is associated and persists it into storage, once, and then continuously.

```
startAutoSave[](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/methods/save/startautosave/)(): Promise<SqliteBunPersister[](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/)>
```
...
| --- | --- | --- |  
This method first runs a single call to the [`save`](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/methods/save/save/) method to ensure the data is in sync with the persisted storage. It then continues to watch for changes to the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/), automatically saving the data to storage.
This method is asynchronous because it starts by making a single call to the asynchronous [`save`](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/methods/save/save/) method. Even for those storage types that are synchronous (like browser storage) it is still recommended that you `await` calls to this method or handle the return type natively as a Promise.
## Example
...
await persister.startAutoSave[](https://tinybase.org/api/persister-sqlite-bun/interfaces/persister/sqlitebunpersister/methods/save/startautosave/)();
