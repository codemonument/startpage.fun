...
It contains a class which, when run in a PartyKit server environment, lets you save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data from a client (that is using the complementary [`persister-partykit-client`](https://tinybase.org/api/persister-partykit-client/) module) to durable PartyKit cloud storage.
This enables synchronization of the same [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) across multiple clients in a PartyKit party room.
Note that both the client and server parts of this [`Persister`](https://tinybase.org/api/the-essentials/persisting-stores/persister/) are currently experimental as PartyKit is new and still maturing.
## See also
...
## Functions
  * [`broadcastChanges`](https://tinybase.org/api/persister-partykit-server/functions/connection/broadcastchanges/) function
