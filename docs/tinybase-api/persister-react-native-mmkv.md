...
The `persister-react-native-mmkv` module of the TinyBase project lets you save and load [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) data to and from a MMKV storage using the [`react-native-mmkv`](https://github.com/mrousavy/react-native-mmkv) module (in an appropriate React Native environment).
## See also
