...
# `hasStoreInStorage`
The `hasStoreInStorage` function returns a boolean indicating whether durable PartyKit storage contains a serialized [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/).
...
| returns  | `Promise<boolean>`  | A promised boolean indicating whether a [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) is present in the storage.  |  
