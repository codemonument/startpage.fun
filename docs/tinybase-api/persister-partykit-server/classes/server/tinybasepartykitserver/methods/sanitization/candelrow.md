...
# `canDelRow`
The `canDelRow` method lets you allow or disallow deletions of a [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) stored on the server, as sent from a client.
...
Return `false` from this method to disallow this [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) from being deleted on the server, or `true` to allow it. The default implementation returns `true` to allow deletion.
## Example
The following implementation will strip out any attempts by the client to delete the 'me' [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) of the 'user' [`Table`](https://tinybase.org/api/store/type-aliases/store/table/):
...
export class MyServer extends TinyBasePartyKitServer[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/) {
  canDelRow[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/candelrow/)(tableId, rowId) {
