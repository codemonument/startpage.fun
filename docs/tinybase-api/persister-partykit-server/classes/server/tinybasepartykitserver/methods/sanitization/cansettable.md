...
# `canSetTable`
The `canSetTable` method lets you allow or disallow any changes to a [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) stored on the server, as sent from a client.
...
This method is passed the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) that the client is trying to change. The `initialSave` parameter distinguishes between the first bulk save of the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to the PartyKit room over HTTP (`true`), and subsequent incremental updates over a web sockets (`false`).
...
Since v4.3.13, the final parameter is the [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) previously stored on the server, if any. Use this to distinguish between the addition of a new [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) (in which case it will be undefined) and the updating of an existing one.
Return `false` from this method to disallow changes to this [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) on the server, or `true` to allow them (subject to subsequent [`canSetRow`](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansetrow/) method, [`canDelRow`](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/candelrow/) method, [`canSetCell`](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansetcell/) method, and [`canSetCell`](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansetcell/) method checks). The default implementation returns `true` to allow all changes.
## Example
...
export class MyServer extends TinyBasePartyKitServer[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/) {
  canSetTable[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansettable/)(tableId, initialSave) {
