...
# `canDelValue`
The `canDelValue` method lets you allow or disallow deletions of a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) stored on the server, as sent from a client.
...
Return `false` from this method to disallow this [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) from being deleted on the server, or `true` to allow it. The default implementation returns `true` to allow deletion.
## Example
The following implementation will strip out any attempts by the client to delete the 'userId' [`Value`](https://tinybase.org/api/store/type-aliases/store/value/):
...
export class MyServer extends TinyBasePartyKitServer[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/) {
  canDelValue[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/candelvalue/)(valueId) {
