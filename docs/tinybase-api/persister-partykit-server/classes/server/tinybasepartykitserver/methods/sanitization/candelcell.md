...
# `canDelCell`
The `canDelCell` method lets you allow or disallow deletions of a [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) stored on the server, as sent from a client.
...
This method is passed the [`Table`](https://tinybase.org/api/store/type-aliases/store/table/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/), and [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) that the client is trying to delete. The `connection` parameter will be the web socket connection of that client. You can, for instance, use this to distinguish between different users.
Return `false` from this method to disallow this [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) from being deleted on the server, or `true` to allow it. The default implementation returns `true` to allow deletion.
## Example
The following implementation will strip out any attempts by the client to delete the 'name' [`Cell`](https://tinybase.org/api/store/type-aliases/store/cell/) of the 'me' [`Row`](https://tinybase.org/api/store/type-aliases/store/row/) of the 'user' [`Table`](https://tinybase.org/api/store/type-aliases/store/table/):
...
export class MyServer extends TinyBasePartyKitServer[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/) {
  canDelCell[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/candelcell/)(tableId, rowId, cellId) {
