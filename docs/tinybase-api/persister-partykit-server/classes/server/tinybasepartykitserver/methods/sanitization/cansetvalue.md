...
# `canSetValue`
The `canSetValue` method lets you allow or disallow any changes to a [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) stored on the server, as sent from a client.

```
canSetValue[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansetvalue/)(
  valueId: string,
  value: Value[](https://tinybase.org/api/store/type-aliases/store/value/),
  initialSave: boolean,
  requestOrConnection: Request | Connection,
  oldValue: ValueOrUndefined[](https://tinybase.org/api/store/type-aliases/store/valueorundefined/),
): Promise<boolean>
```
...
This method is passed the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) [`Id`](https://tinybase.org/api/common/type-aliases/identity/id/) that the client is trying to change - as well as the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) itself. The `initialSave` parameter distinguishes between the first bulk save of the [`Store`](https://tinybase.org/api/the-essentials/creating-stores/store/) to the PartyKit room over HTTP (`true`), and subsequent incremental updates over a web sockets (`false`).
...
Since v4.3.13, the final parameter is the [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) previously stored on the server, if any. Use this to distinguish between the addition of a new [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) (in which case it will be undefined) and the updating of an existing one.
Return `false` from this method to disallow changes to this [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) on the server, or `true` to allow them. The default implementation returns `true` to allow all changes.
## Example
The following implementation will strip out any attempts by the client to update the 'userId' [`Value`](https://tinybase.org/api/store/type-aliases/store/value/) after the initial save:
...
export class MyServer extends TinyBasePartyKitServer[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/) {
  canSetValue[](https://tinybase.org/api/persister-partykit-server/classes/server/tinybasepartykitserver/methods/sanitization/cansetvalue/)(valueId, value, initialSave) {
